...
local RECORDTIME_ACK_EVENT = "HOST_UART_RECORDTIME_ACK"
local RECORDTIME_SET_DONE = "HOST_UART_RECORDTIME_SET_DONE"
local IPCSTATUS_ACK_EVENT = "HOST_UART_IPCSTATUS_ACK"
local IPCSTAT_ACK_EVENT = "HOST_UART_IPCSTAT_ACK"
...
local FRAMERATE_QUERY_DONE = "HOST_UART_FRAMERATE_QUERY_DONE"
local FRAMERATE_SET_DONE = "HOST_UART_FRAMERATE_SET_DONE"
local RECORDCTRL_SET_DONE = "HOST_UART_RECORDCTRL_SET_DONE"
local WLED_ACK_EVENT = "HOST_UART_WLED_ACK"
local TFFORMAT_ACK_EVENT = "HOST_UART_TFFORMAT_ACK"
local PERSONDET_ACK_EVENT = "HOST_UART_PERSONDET_ACK"
local PERSONDET_SET_DONE = "HOST_UART_PERSONDET_SET_DONE"
local MIC_QUERY_DONE = "HOST_UART_MIC_QUERY_DONE"
local MIC_SET_DONE = "HOST_UART_MIC_SET_DONE"
local SOFTPHOTO_QUERY_DONE = "HOST_UART_SOFTPHOTO_QUERY_DONE"
local SOFTPHOTO_SET_DONE = "HOST_UART_SOFTPHOTO_SET_DONE"
...
local function try_wled_line(line)
    ...
    if line:match("^%+WLED:ERROR") then
        sys.publish(WLED_ACK_EVENT, { ok = false })
        return true
    end
    ...
    sys.publish(WLED_ACK_EVENT, { ok = true, on = n })
    return true
end
...
local function try_tfformat_line(line)
    ...
    if line:match("^%+TFFORMAT:ERROR") then
        ...
        sys.publish(TFFORMAT_ACK_EVENT, { phase = "error", ret = ret })
        return true
    end
    if line:match("^%+TFFORMAT:STARTED") then
        sys.publish(TFFORMAT_ACK_EVENT, { phase = "started" })
        return true
    end
    if line:match("^%+TFFORMAT:OK") then
        local reboot = line:match("reboot=(%d+)") or "0"
        sys.publish(TFFORMAT_ACK_EVENT, { phase = "ok", reboot = tonumber(reboot) or 0 })
        return true
    end
    return false
end
...
local function try_recordtime_line(line)
    ...
    if snap.query then
        ...
        sys.publish(RECORDTIME_ACK_EVENT, snap)
    elseif snap.set then
        sys.publish(RECORDTIME_SET_DONE, snap)
    end
    return true
end
...
local function try_encode_ok_tail(line)
    ...
    if state.mic_rows ~= nil then
        ...
        state.mic_rows = nil
        sys.publish(MIC_QUERY_DONE, rows)
        return true
    end
    return false
end
...
local function try_softphotoset_line(line)
    if line and line:match("^%+SOFTPHOTOSET:OK") then
        sys.publish(SOFTPHOTO_SET_DONE, { ok = true })
        return true
    end
    if line and line:match("^%+SOFTPHOTOSET:ERROR") then
        sys.publish(SOFTPHOTO_SET_DONE, { ok = false })
        return true
    end
    return false
end
...
local function try_softphoto_line(line)
    ...
    if en then
        sys.publish(SOFTPHOTO_QUERY_DONE, {
            enable = tonumber(en) or 0,
    ...
    if line:match("^%+SOFTPHOTO:ERROR") then
        sys.publish(SOFTPHOTO_QUERY_DONE, { parsed = false, error = true })
        return true
    end
    return false
end
...
local function try_framerate_line(line)
    ...
    if line == "+FRAMERATE:END" then
        ...
        sys.publish(FRAMERATE_QUERY_DONE, rows)
        return true
    end
    ...
    if okCam then
        sys.publish(FRAMERATE_SET_DONE, {
            ok = true,
    ...
    if line:match("^%+FRAMERATE:ERROR") then
        sys.publish(FRAMERATE_SET_DONE, { ok = false, error = true })
        return true
    end
    return false
end
...
local function try_recordctrl_line(line)
    ...
    if ok1 then
        sys.publish(RECORDCTRL_SET_DONE, { ok = true, start = 1, max_sec = tonumber(maxSec) or 60 })
        return true
    end
    local ok0, reason = line:match("^%+RECORDCTRL:OK,0,reason=(.+)$")
    if ok0 then
        sys.publish(RECORDCTRL_SET_DONE, { ok = true, start = 0, reason = reason or "cloud" })
        return true
    end
    if line:match("^%+RECORDCTRL:ERROR") then
        sys.publish(RECORDCTRL_SET_DONE, { ok = false, error = true })
        return true
    end
    return false
end
...
local function try_persondet_line(line)
    ...
    if enable then
        ...
        sys.publish(PERSONDET_ACK_EVENT, state.host_person_detect)
        return true
    end
    ...
    if okEn then
        sys.publish(PERSONDET_SET_DONE, { ok = true, enable = tonumber(okEn) or 0 })
        return true
    end
    if line:match("^%+PERSONDET:ERROR") then
        sys.publish(PERSONDET_SET_DONE, { ok = false, error = true })
        return true
    end
    return false
end
...
function commitHostIpcCloudStat(snap)
    ...
    sys.publish(IPCSTAT_ACK_EVENT, snap)
    return snap
end
...
function queryHostRecordTime(timeoutMs)
    return host_query(timeoutMs, {
        ...
        ack_event = RECORDTIME_ACK_EVENT,
        log_extra = "mqtt_start",
...
function setHostRecordTime(opts)
    ...
    local ok, e = pcall(function()
        ...
        local got, rsp = sys.waitUntil(RECORDTIME_SET_DONE, timeoutMs)
...
function queryHostFramerate(opts)
    ...
    return host_query(opts.timeout_ms, {
        ...
        at_cmd = at_cmd,
        ack_event = FRAMERATE_QUERY_DONE,
...
function setHostFramerate(opts)
    ...
    local ok, e = pcall(function()
        ...
        local got, rsp = sys.waitUntil(FRAMERATE_SET_DONE, timeoutMs)
        if not got or type(rsp) ~= "table" then
            okSet, msg = false, "timeout"
            return
        end
...
function recordCtrlStart(opts)
    ...
    local got, rsp = sys.waitUntil(RECORDCTRL_SET_DONE, timeoutMs)
    if not got or type(rsp) ~= "table" then
...
function setHostPersonDetect(opts)
    ...
    local ok, e = pcall(function()
        ...
        local got, rsp = sys.waitUntil(PERSONDET_SET_DONE, timeoutMs)
        if not got or type(rsp) ~= "table" then
            okSet, msg = false, "timeout"
            return
        end
...
function setHostMic(opts)
    ...
    local ok, e = pcall(function()
        ...
        local got, rsp = sys.waitUntil(MIC_SET_DONE, timeoutMs)
        if not got or type(rsp) ~= "table" then
            okSet, msg = false, "timeout"
            return
        end
...
function queryHostSoftPhoto(timeoutMs)
    return host_query(timeoutMs, {
        ...
        default_timeout = 8000,
        at_cmd = "AT+SOFTPHOTO?",
        ack_event = SOFTPHOTO_QUERY_DONE,
        log_extra = "spQ",
...
function setHostSoftPhoto(opts)
    ...
    local ok, e = pcall(function()
        ...
        local got, rsp = sys.waitUntil(SOFTPHOTO_SET_DONE, timeoutMs)
        if not got or type(rsp) ~= "table" then
            okSet, msg = false, "timeout"
            return
        end
...
function formatHostTfCard(opts)
    ...
    local okRun, errRun = pcall(function()
        ...
        while (os.time() * 1000) < deadline do
            ...
            local got, val = sys.waitUntil(TFFORMAT_ACK_EVENT, slice)
