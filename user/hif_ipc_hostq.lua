-- ================================================================
-- Filename : hif_ipc_hostq.lua
-- Module   : RECORD / FPS / MIC / SOFTPHOTO / TFCARD query/set，由 hif_ipc.bind
...
function bind(C, H)
    ...
    local function t31xRecOn()
        if tonumber(state.t31x_rec_active) == 1 then
            return 1
        end
        local cloud = state.host_ipc_cloud_stat
        if type(cloud) == "table" and tonumber(cloud.recordingt31x) == 1 then
            return 1
        end
        return 0
    end
