-- ================================================================
-- Filename : hif_cmd_usb.lua
-- Module   : AT+USBRESET/RNDIS/USBRECOVERY handler，由 hif_cmd.bind
...
function bind(C)
    ...
    local function t31xRestBlock()
        local cfg = hostUsbCfg()
        if cfg.block_usb_reset_when_t31x_rest == false then
            return false
        end
    ...
    local function execUsbReset(tag, cfg, do_fn)
        ...
        sys.taskInit(function()
            ...
            if ok and cfg.notify_t31x_usb_state ~= false and usbInserted() then
                sys.wait(notify_ms)
                pushUsbIdle(1)
            end
    ...
    local function uartUsbReset(cmd)
        ...
        execUsbReset("USBRESET", cfg, function()
            ...
            if cfg.usb_reset_soft_rebind == false then
                ...
                if t31x then
                    local pok, _, pms = pcall(t31x.pulseUsbDebugEn, { highMs = cfg.usb_debug_en_pulse_ms })
                    if pok and pms then
