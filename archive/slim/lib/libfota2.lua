...
local function logIotErrorBody(url, body)
    ...
    log.info(LOG_TAG, "iot rsp")
    local jsonBody, ok = json.decode(body)
    if ok == 1 and isJsonBody(body) then
        log.info(LOG_TAG, "c", jsonBody.code)
    else
        log.info(LOG_TAG, "bad rsp", type(body))
    end
end
...
local function fotaTask(cbFnc, opts)
    ...
    elseif code == 401 or code == 403 then
        log.error(LOG_TAG, "http", code)
        logIotErrorBody(opts.url, body)
        ret = 3
    elseif code >= 300 then
        log.error(LOG_TAG, "http", code, "b", body)
        logIotErrorBody(opts.url, body)
        ret = 3
    else
        log.error(LOG_TAG, "http", code, "b", body)
...
local function buildIotUpgradeUrl(opts)
    ...
    if not iotVer then
        log.error(LOG_TAG, "ver?", opts.version)
        return false
    end
    if iotVer ~= opts.version then
        log.info(LOG_TAG, "iot", opts.version, "→", iotVer)
    end
    opts.version = iotVer
...
function libfota2.request(cbFnc, opts)
    ...
    log.info(LOG_TAG, "req", opts.version)
