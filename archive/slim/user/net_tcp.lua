--- TCP 低功耗长连接（仅 LOW_POWER_WAKEUP_CFG.mode="tcp" 时启用）
-- t3x AT+SERVCREATE / SERVCLOSE：登录、心跳、wake_hex 唤醒
-- 门禁：lib/low_power_wakeup.lua；与 net_mqtt.lua（MQTT 模式）二选一
-- @module net_tcp
...
local function isEnabled()
    ...
    if ok and lpw and lpw.allowTcpChannel then
        return lpw.allowTcpChannel()
    end
    return false
end
...
function applyChannel(ch)
    if not isEnabled() then
        log.info(LOG_TAG, "disabled, skip applyChannel")
        return false
    end
