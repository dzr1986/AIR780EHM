...
现网地址（实机文档 `doc/MQTT_862323084068314.md`、`http_server/HOW_IT_RUNS.md`）：
...
**启动顺序**（与 `doc/CALL_GRAPH.md` 一致，简化）：
...
- 录像会话与二次触发见 `doc/PIR_CTRL_FLOW.md`、`doc/WORK_MODE_PERSON_DETECT_PIR.md`。
...
```
> MQTT **不传文件**；同任务闭环共用 2013 的 `messageId`。详见 `doc/MQTT_CLIP_UPLOAD_CLOSED_LOOP.md`。
...
```
版本号双轨：脚本版 `001.000.151` ↔ IoT 版 `内核号.001.151`（`2034.001.151`），服务器统一 IoT 格式。详见 `ota_server/docs/OTA_FLOW.md`。
...
**UART AT**（真源 `doc/UART_AT_COMMANDS.md`）：`AT`/`ATI` 握手、`AT+RECORD`/`AT+STOPRECORD`、`AT+TFCARD?`/`AT+TFFORMAT`、`AT+GB28181?`、`AT+HOSTEVT?`/`HOSTEVTCLR`、`AT+PIRSTAT?`/`PIRCLR`、`AT+MQTTCFG`/`AT+SERVCREATE`、`AT+UPLOADVIDEO`/`UPLOADPROGRESS`/`UPLOADRESULT`、URC（`+PIR`/`IPCALERT`/`+WLED`…）。

**GPIO 信号**：PIR→GPIO30 中断；唤醒低脉冲 GPIO29→T31x PB27；T31x 供电、充电/USB VBUS、烧录引脚 GPIO28；详见 `doc/T31X_CAT1_GPIO.md` §1.1 全表。
...
| 主题 | 文档 |
|------|------|
| 协议总览 | `doc/MQTT_PROTOCOL.md`、`doc/UART_AT_COMMANDS.md`、`doc/T31X_CAT1_AT_COMMAND_SPEC.md` |
| 协作框架 | `doc/T31X_4G_FRAMEWORK.md`、`doc/T31X_4G_AT_INTERACTION.md`、`doc/T31X_IPC_4G_INTERACTION.md` |
| 固件模块 | `doc/LUA_MODULES.md`、`doc/CAT1_MODULE_FRAMEWORK.md`、`doc/CALL_GRAPH.md`、`doc/modules/README.md` |
| 视频上传 | `doc/MQTT_2013_1013_UPLOAD_VIDEO.md`、`doc/MQTT_CLIP_UPLOAD_CLOSED_LOOP.md`、`video_upload_server/README.md` |
| OTA | `ota_server/docs/OTA_SERVER.md`、`OTA_FLOW.md`、`OTA_PROTOCOL.md`、`OTA_CONSOLE_UPGRADE.md` |
| 低功耗/PIR/电源 | `doc/WORK_MODE_PERSON_DETECT_PIR.md`、`doc/PIR_CTRL_FLOW.md`、`doc/LOW_BATTERY_AND_LOW_POWER.md`、`doc/CAT1_LOWPWR_MQTT_TCP_STRATEGY.md` |
| 硬件/GPIO | `doc/T31X_CAT1_GPIO.md`、`doc/CONFIG.md`、`doc/KEY_GPIO.md`、`doc/LED_INDICATORS.md` |
| 工具链 | `tools/README.md`、`doc/CAT1_FLASH_FLOW.md`、`doc/CAT1_FLASH_TOOL.md` |
