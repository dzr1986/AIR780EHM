# T31x IPC ↔ Cat.1 监督模块架构

> **原则**：IPC 与 Cat.1 **两侧独立实现**，通过 **共享契约**（头文件 + Lua 镜像 + 本文档）对齐行为。  
> **真源路径**：IPC 仓库 `app/cat1/`；Cat.1 `user/`；契约文档 `doc/t31x/T31X_IPC_ALERT_CONTRACT.md`。
...
完整码表与策略见 **[T31X_IPC_ALERT_CONTRACT.md](T31X_IPC_ALERT_CONTRACT.md)**。
...
**联调日志**：Cat.1 缩略 log tag 已还原，见 [CAT1_LOG_TAGS.md](../overview/CAT1_LOG_TAGS.md)（IPC 镜像：`docs/cat1_log_tags.md`）。
...
- [ ] Cat.1 烧录后 `T31X_IPC_ALERT` 能出 1004
...
- [T31X_IPC_ALERT_CONTRACT.md](T31X_IPC_ALERT_CONTRACT.md) — 码表与策略
- [T31X_IPC_CAT1_SUPERVISION.md](T31X_IPC_CAT1_SUPERVISION.md) — 三层监督与历史分析
- [T31X_IPC_CLOUD_EXCEPTION_REPORT.md](../mqtt/T31X_IPC_CLOUD_EXCEPTION_REPORT.md) — §4.x 异常分类
- [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](T31X_IPC_EXCEPTION_MQTT_UPLINK.md) — 后台上行格式

IPC 仓库镜像：`docs/t31x_ipc_supervision_module.md`、`docs/ipc_alert_contract.h`
