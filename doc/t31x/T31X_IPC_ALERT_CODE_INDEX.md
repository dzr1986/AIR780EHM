...
> **用途**：联调 / Code Review 时按 `alertCode` 反查 T31x 触发点与 Cat.1 MQTT 出口  
...
> **关联**：[T31X_IPC_CAT1_SUPERVISION.md](T31X_IPC_CAT1_SUPERVISION.md) · [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](T31X_IPC_EXCEPTION_MQTT_UPLINK.md)

---

## 1. 公共链路（所有 T31x IPCALERT 共用）
...
| **no_person** | L13 | `record_notify.c:77` | NULL | 停录 reason=no_person；**不发** `AT+RECORD=0` | **是** | 1004 + 1011 + `syncStopFromT31x` |
...
| **recordctrl_fail** | L19 | `cloud_remote_ctrl.c:204` start<br>`cloud_remote_ctrl.c:219` stop<br>**Lua** `net_mqtt.lua:930` | `start` / `stop` / rmsg | `AT+RECORDCTRL` 开停录失败；2012 直连失败 | **是** | T31x 路径 1004+1011；4G 路径仅 `pubIpcAlert` |
| **ipcpoweroff_busy** | L20 | `uart_host_cmd.c:699` off=1<br>`uart_host_cmd.c:711` off=0 | NULL | `ipc_power_off_request` 返回忙 | 否 | 仅 1004 |

### 2.1 Cat.1 本地 alertCode（不经 T31x `AT+IPCALERT`）
...
`net_mqtt.lua:1799–1806` — 下列 alertCode 在发 **1004** 后会再调 `publishT31xRecordStop(alertCode, …)` → **1011** `source=t31x` `reason=<alertCode>`：

| alertCode | 1011 reason | 清 4G 会话 |
| --- | --- | --- |
| `no_person` | `no_person` | `pir_ctrl.syncStopFromT31x` L1811–1812 |
...
**1011 发布实现**：`net_mqtt.lua:2023–2030` `publishT31xRecordStop` → `pubPirStop` L1987–2019。

**不经 IPCALERT、直接 1011 的录像 reason**（`record_notify.c` → `AT+RECORD=0` → `host_uart.lua:604–628` → `T31X_RECORD_STOP`）：  
`disk_full` · `time_sync` · `not_inited` · `no_record` · `open_failed` · `no_iframe` · `no_stream` · `failed` · `done` · `timer` · `cloud` · `pir_retrigger` · `allday_wait_person` 等 — 见 [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)。
...
*IPC 仓库镜像：`ipc_device_gb28181/docs/t31x_ipc_alert_code_index.md`*
