...
> **读者**：固件 / 联调 / 平台  
> **工程**：780EHM_PJ（Cat.1 Lua）+ T31x IPC（`ipc_device_gb28181`）  
> **关联**：[T31X_IPC_CLOUD_EXCEPTION_REPORT.md](../mqtt/T31X_IPC_CLOUD_EXCEPTION_REPORT.md) · [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](T31X_IPC_EXCEPTION_MQTT_UPLINK.md) · [T31X_IPC_SUPERVISION_MODULE.md](T31X_IPC_SUPERVISION_MODULE.md) · [MQTT_PROTOCOL.md](../mqtt/MQTT_PROTOCOL.md)  
...
| **状态型**（1003 IPCSTAT） | ★★★★☆ | 9 个扩展字段 + 周期刷新；T31x 休眠时 IPCSTAT 可能不拉 |
...
**平台原则**：告警入库看 **1004**；**当前是否正常** 以 **最新 1003** 为准，勿等待「恢复事件」。详见 [T31X_IPC_EXCEPTION_MQTT_UPLINK.md §5](T31X_IPC_EXCEPTION_MQTT_UPLINK.md)。
...
sequenceDiagram
    ...
    UART->>HU: 上行 AT（T31x→4G）
    HU->>HU: uart_ipc_alert_notify
    HU->>SUP: sys.publish(T31X_IPC_ALERT) → pubAlert
    SUP->>MQ: publishAlert → pubUplink
    MQ->>BE: 1004 action=ipc_alert
    SUP->>HU: scheduleIpcCloudStatRefresh(force=true)
    opt map1011 含 code
        SUP->>MQ: syncStopFromT31x + publishT31xRecordStop
        MQ->>BE: 1011 source=t31x reason=code
    end
```
...
sequenceDiagram
    participant MQ as net_mqtt.lua
    participant HU as host_uart.lua
    participant T31x as uart_host_cmd.c
    participant BE as MQTT 后台
    ...
    Note over HU: T31x idle 时跳过（省电）
    HU->>T31x: AT+IPCSTAT?（非 idle 或告警后 force）
    T31x->>HU: +IPCSTAT:ipcReady=,gb28181Online=,...
...
触发条件
  ...
  └─ T31x AT+RECORD=0 → syncStopFromT31x + 1011
...
对账逻辑（host_uart.reconcileHostRecordSession）
  ...
  3. 若 T31x 已停 → syncStopFromT31x + T31X_RECORD_STOP → 1011
```
...
MP4 写盘失败 / 正常停录
  ...
  → T31X_RECORD_STOP → net_mqtt publishT31xRecordStop → 1011 source=t31x
```
...
| `app/cat1/uart_host_cmd.c` | T31x Host AT：`IPCSTAT?` / `RECORD?` / `TIMESET` / … |
...
> **完整行号速查表**（含公共链路、map1011、按文件反查、rg 命令）→ [T31X_IPC_ALERT_CODE_INDEX.md](T31X_IPC_ALERT_CODE_INDEX.md)
...
T31x `ipc_supervision_build_stat()` 填充，4G `AT+IPCSTAT?` 拉取后并入 **1003**：
...
| `recordingT31x` | `record_get_runtime_status()` running/active |

**刷新节奏**：

- 每帧 1003 `onPublished` → `scheduleIpcCloudStatRefresh(false)`（T31x **idle** 时跳过，省电）
...
| **T31x 休眠 IPCSTAT 滞后** | **部分缓解** | 周期 1003 仍 skip idle；**告警后 force 刷新** |
...
5. **录像漂移**：4G 仍在录但 T31x 已停 → 应在对账后出现 **1011** + `recordingT31x=0`
...
| 文档 | 内容 |
| --- | --- |
| [T31X_IPC_SUPERVISION_MODULE.md](T31X_IPC_SUPERVISION_MODULE.md) | **模块化架构**、两侧对称、API、迁移对照 |
| [T31X_IPC_ALERT_CONTRACT.md](T31X_IPC_ALERT_CONTRACT.md) | alertCode 契约表 + map1011/reconcile |
| [T31X_IPC_CLOUD_EXCEPTION_REPORT.md](../mqtt/T31X_IPC_CLOUD_EXCEPTION_REPORT.md) | 异常分类、§4.x 修复表 |
| [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](T31X_IPC_EXCEPTION_MQTT_UPLINK.md) | 后台 dataType/JSON、恢复态 |
| [T31X_IPC_ALERT_CODE_INDEX.md](T31X_IPC_ALERT_CODE_INDEX.md) | alertCode 源码行号速查 |
| [UART_AT_COMMANDS.md](../mqtt/UART_AT_COMMANDS.md) | IPCSTAT / IPCALERT / RECORD? AT 格式 |
| [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md) | 1010/1011 录像时序 |
...
*IPC 仓库镜像：`ipc_device_gb28181/docs/t31x_ipc_cat1_supervision.md`*
