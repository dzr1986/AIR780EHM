# T31x App ↔ Cat.1 Lua 通讯完善度分析

> T31x 主程序（`app/cat1/`）与 Air780 固件（`user/` + `lib/`）双向 UART AT 通讯的完整性评估。  
> 关联：[T31X_IPC_4G_INTERACTION.md](T31X_IPC_4G_INTERACTION.md)、[T31X_HOSTEVT_SLEEP.md](T31X_HOSTEVT_SLEEP.md)、[UART_AT_COMMANDS.md](../mqtt/UART_AT_COMMANDS.md)

**版本**：v1.5 · 2026-06-06

---

## 0. USB 插入与 T31x 休眠互斥（780EHM_PJ）
...
配置：`HOST_USB_CFG`（`config.lua`）。完整时序：[T31X_LOW_POWER.md §2.1](../power/T31X_LOW_POWER.md)。
...
**一句话**：原「部分完善 / 待补」项已在 v1.5 落地；`AT+LOWPOWER` 仍由 4G 内部使用、T31x 走 `HOSTIDLE`（设计如此，非缺口）。
...
4G  Lua: app.lua / pir_ctrl / net_mqtt / host_uart.lua
     │  uart_bridge (115200)
     │  t31x_ctrl: GPIO 供电 + GPIO29 唤醒脉冲
     ▼
T31x: serial.c → api.c (T31x→4G) + uart_host_cmd.c (4G→T31x)
     runtime.c / host_event.c / media_ops.c
```

| 通道 | 机制 | 互斥 / 拆包 |
|------|------|-------------|
| T31x→4G | `serial_request` + `client_request` | `tx_lock` 仅保护写；等待应答时不持锁，避免 Host 行死锁 |
| 4G→T31x | `uart_bridge.sendString` 发 AT；`host_process_line` 解析 T31x 应答 | 首条 AT 触发 `HOST_UART_FIRST_AT` / 开机铃 |
| GPIO | `ntfHost` → `set_pending_wake` + `pulseMcuInt` | T31x `gpio_wait_event` 后再 `AT+HOSTEVT?` 校验 |
...
### 3.1 T31x → 4G（`api.c` 发，`host_uart.lua` 收）
...
### 3.2 4G → T31x（`host_uart.lua` 发，`uart_host_cmd.c` 收）

| AT / URSP | T31x 处理 | 完善度 |
|-----------|----------|--------|
| `AT+TIMESET=` | `time_sync_apply_unix` | ✅ |
| `AT+PLAYSOUND=` | 开机铃/提示音 | ✅ |
| `AT+IPCPOWEROFF` | 低功耗断电流程 | ✅（需 `WITH_T31X_LOW_POWER`） |
| `AT+IPCSTATUS?` | 运行状态字符串 | ✅ |
| `AT+RECORD?` / `TFCARD?` | 4G 主动查 T31x 写盘/卡 | ✅（双格式解析在 Lua 侧） |
...
| 方向 | 格式 | 用途 |
|------|------|------|
| T31x → 4G 上报 | `+RECORD:1,active=1` / `+RECORD:0,reason=*` | 4G 会话状态、MQTT 触发 |
| 4G → T31x 查询 | `+RECORD:running=,active=,ch=,reason=` | `queryHostRecord()` 查真实写盘 |

`host_uart.lua` 的 `parse_record_line()` 已兼容两种格式。详见 [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)。
...
sequenceDiagram
    ...
    participant HU as host_uart
    participant TC as t31x_ctrl

    loop 每 1s 无 GPIO
        T->>HU: AT+HOSTEVT?
        alt has_event=0
            T->>HU: AT+HOSTIDLE=1
            HU->>TC: enterSleep(skipPendingWorkCheck)
...
| 跳过重复 pending 检查 | — | `enterSleep({ skipPendingWorkCheck=true })` |
...
1. **HOSTEVT 休眠 + 媒体**：`HOSTEVT?` 含 `recording/action/max_sec/last_stop`；T31x `media_dispatch` 主读 HOSTEVT。
2. **MQTT pending**：`hasHostQueue()`（2006/2007 队列 + 云端停录）；`types_mask=0x0F`；T31x `pending=mqtt` 仅 block 休眠。
3. **`+CAT1:MQTT`**：`LED_CFG.notify_t31x_net_led=true` 时驱动 T31x NET_STAT_LED（Cat.1 MQTT 在线态覆盖）。
...
| 4G | `pir_ctrl.lua` | `requestStopFromCloud` → `requestT31xStopRecord`（唤醒 T31x 停录） |
| 4G | `host_event.lua` | `isDispatchable` 跳过 mqtt |
| 4G | `config.lua` | `types_mask=0x0F` |
| T31x | `media_ops.c` | 优先 HOSTEVT，回退 PIRSTAT |
| T31x | `host_event.c` | 解析 media 字段；mqtt 空闲轮询 skip |
| T31x | `uart_host_cmd.c` | 处理 `+CAT1:MQTT,n` |
| T31x | `gpio_ctrl_interface.c` | `gpio_net_stat_set_cat1_mqtt` |

### 5.3 仍须注意（非缺口）

| 项 | 说明 |
|----|------|
| 双端 HOSTEVT 开关 | 须同时对齐 `WITH_T31X_HOSTEVT_SLEEP` ↔ `HOST_EVT_ENABLE` |
| PIRSTAT 回退路径 | HOSTEVT 查询失败时仍读 PIRSTAT，运维日志可能见 `fallback PIRSTAT` |
| `AT+LOWPOWER` | 4G 内部 API；T31x 休眠统一 `HOSTIDLE=1` |
...
| 低功耗 | `WITH_T31X_LOW_POWER=yes` | `LOW_POWER_ENABLE=1` |
| HOSTEVT 休眠 | `WITH_T31X_HOSTEVT_SLEEP=yes` | `HOST_EVT_ENABLE=1` |
| HOSTEVT types | — | `types_mask=0x0F`（含 mqtt） |
| Cat.1 网络灯 | `WITH_NET_STAT_LED` | `LED_CFG.notify_t31x_net_led`（可选） |

**必须双端同时开启**，否则 T31x 发 `HOSTIDLE=1` 可能收到 `NOT_SUPPORTED`，或 4G 侧 `enterSleep` 与 T31x 轮询不同步。
...
| 模块 | 文件 | 职责 |
|------|------|------|
| AT 分发 | `user/host_uart.lua` | T31x 入站 AT、HOSTEVT/PIRSTAT/RECORD |
| 事件汇总 | `lib/host_event.lua` | `summarize` / `isDispatchable` |
| 唤醒脉冲 | `user/t31x_ctrl.lua` | `ntfHost` / `enterSleep` |
| PIR 策略 | `user/pir_ctrl.lua` | 录像会话、`syncStopFromT31x` |
...
| 模块 | 文件 | 职责 |
|------|------|------|
| Bootstrap / AT 客户端 | `app/cat1/api.c` | T31x→4G 全部 `client_request` |
| 串口 | `app/cat1/serial.c` | `serial_request`、Host 行拆包 |
| 4G→T31x Host AT | `app/cat1/uart_host_cmd.c` | `TIMESET` / `PLAYSOUND` / `RECORD?` 等 |
...
- [ ] 抓拍后 T31x `SNAPSHOT notify sent`；4G MQTT 1010 `snapshot_saved`
...
- [ ] T31x 休眠时平台发 2006/2007 → 4G 日志 `HOST 下行入队`；T31x 上电后 `HOST 下行出队`
- [ ] `notify_t31x_net_led=true` 时 MQTT 断连 → T31x 收到 `+CAT1:MQTT,0`，NET_STAT 闪烁
...
| 文档 | 说明 |
|------|------|
| [T31X_IPC_4G_INTERACTION.md](T31X_IPC_4G_INTERACTION.md) | 端到端总览、rest、优化记录 |
| [T31X_HOSTEVT_SLEEP.md](T31X_HOSTEVT_SLEEP.md) | HOSTEVT 四条 AT；**§2 精简 vs 宽表**（HOSTEVT / PIRSTAT 分工） |
| [T31X_HOSTEVT_PROTOCOL.md](T31X_HOSTEVT_PROTOCOL.md) | GPIO 脉冲时序 |
| [UART_AT_COMMANDS.md](../mqtt/UART_AT_COMMANDS.md) | AT 全表与 RECORD 双格式 |
| [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md) | 录像 MQTT 1010/1011 |
