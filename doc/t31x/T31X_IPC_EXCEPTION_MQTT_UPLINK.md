# T31x IPC 异常 → MQTT 后台上行协议与恢复态说明

> **读者**：MQTT 后台 / App 服务端 / 联调  
> **工程**：780EHM_PJ（Cat.1 Lua）+ T31x IPC（`ipc_device_gb28181`）  
> **关联**：[MQTT_PROTOCOL.md](../mqtt/MQTT_PROTOCOL.md) · [T31X_IPC_CLOUD_EXCEPTION_REPORT.md](../mqtt/T31X_IPC_CLOUD_EXCEPTION_REPORT.md) · [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)  
> **更新**：2026-06-26
...
T31x **不直连 MQTT**。所有 App 侧可见上行均由 **Cat.1** 发布。
...
| **1010** | `event` | **结果型** | PIR 触发 / 抓拍 / T31x 开录活跃 |
...
3. **录像会话**：以 **1010/1011** + 1003 的 `recordingT31x` 联合判断；1011 后 4G 侧 `session.recording` 会清掉。
...
sequenceDiagram
    ...
    Note over T31x,BE: 路径 A — 状态型（周期反映恢复）
    T31x->>UART: AT+IPCSTAT? 应答 +IPCSTAT:...
    UART->>MQ: 缓存 host_ipc_cloud_stat
    MQ->>BE: 1003 /status（含 ipcReady/gb28181Online/...）
    Note over BE: 恢复后下一帧 1003 字段变回 1/0 正常值

    Note over T31x,BE: 路径 B — 事件型告警
    T31x->>UART: AT+IPCALERT=code,detail
    UART->>MQ: T31X_IPC_ALERT 事件
    MQ->>BE: 1004 /event action=ipc_alert
    opt 部分 code
        MQ->>BE: 1011 /event source=t31x reason=code
    end

    Note over T31x,BE: 路径 C — 录像结果
    T31x->>UART: AT+RECORD=0,reason=...
    UART->>MQ: T31X_RECORD_STOP
    MQ->>BE: 1011 /event source=t31x
```
...
**与 IPC 异常相关的扩展字段**（来自 T31x `AT+IPCSTAT?`，1003 发布后异步刷新）：

| 字段 | 类型 | 含义 | 异常时典型值 | 恢复后 |
| --- | --- | --- | --- | --- |
| `ipcReady` | 0/1 | T31x 生命周期 ready 且 Cat.1 链路可用 | `0`（未就绪/Cat.1 init 失败） | `1` |
...
| `recordingT31x` | 0/1 | T31x 真实写盘 | `0`/`1` | 随录会话变化 |
...
T31x `AT+IPCALERT=<alertCode>[,detail]` → Cat.1 `pubIpcAlert`。
...
**同次可能追加 1011**（`source=t31x`，`reason=alertCode`）的 alertCode：
...
或 T31x 写盘后：`pirStatus=t31x_active` 等（见 [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)）。
...
| reason（节选） | 来源 | 后台含义 |
| --- | --- | --- |
| `time_sync` / `time_sync_fail` | T31x 开录/校时 | 未校时无法录 |
...
**恢复态**：1011 表示**会话已结束**；下一条 **1003** 中 `recordingT31x=0`；4G `session.recording=false`。新 PIR/2012 会重新发 1010。
...
| --- | --- |
| `ret` | 0 成功；-1 失败 |
| `runtimeApply` | **1**=T31x `SetFramerate` 成功；**0**=ini 已写但运行时未生效 |
...
| § | 场景 | T31x 侧触发 | MQTT 后台收到 | Topic 后缀 | 恢复后链路如何更新 |
| --- | --- | --- | --- | --- | --- |
...
| 4.3 | UART RECORD 通知失败 | record_notify 重试后 | **1004** `uart_notify_fail` | `event` | **1003** 周期对账 + **1011** sync；`recordingT31x` 对齐 |
...
| 4.3 | runtime worker 异常 | wait wakeup fail | **1004** `runtime_wakeup_fail` | `event` | Cat.1/T31x 重启后 **1003** `cat1Link=1` |
...
| 机制 | 更新什么 | 延迟 |
| --- | --- | --- |
| **1003 周期上报** | `ipcReady` · `cat1Link` · `gb28181Online` · `tfPresent` · `timeSynced` · `personDetectAvailable` · `recordingT31x` | ≤ `interval`（默认 30s） |
| **1003 发布后** | 异步 `AT+IPCSTAT?` 刷新缓存（下一条 1003 更准确） | 同周期内 |
| **USB 状态变化** | 立即 **1003**（`usbRecovery*`） | 即时 |
| **1011 / syncStopFromT31x** | 4G 录像会话 `recording=false` | IPCALERT/RECORD=0 后即时 |
| **reconcileHostRecordSession** | 1003 后若 4G 仍在录但 T31x 已停 → 补 **1011** | 每帧 1003 后 task |
...
on MQTT message:
  ...
  if dataType == "1003":
      update_device_snapshot(deviceNo, {
        ...
        timeSynced, personDetectAvailable, recordingT31x,
...
| 环节 | 路径 |
| --- | --- |
| T31x 告警/状态 | `ipc_device_gb28181/app/cat1/ipc_cloud_report.c` |
| T31x → UART 通知 | `record_notify.c` · `cloud_remote_ctrl.c` · `network_module.c` |
| Cat.1 MQTT 发布 | `/mnt/share/user/net_mqtt.lua`（镜像 `docs/4g_lua/user/`） |
| Cat.1 UART 解析 | `/mnt/share/user/host_uart.lua` |
| 异常分类总表 | [T31X_IPC_CLOUD_EXCEPTION_REPORT.md](../mqtt/T31X_IPC_CLOUD_EXCEPTION_REPORT.md) |

---

*IPC 仓库镜像：`ipc_device_gb28181/docs/t31x_ipc_exception_mqtt_uplink.md`*
