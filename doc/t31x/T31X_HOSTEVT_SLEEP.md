# T31x 休眠前事件查询（HOSTEVT 五条 AT）

> T31x 与 4G 协处理器之间，**事件汇总、消费、休眠、轮询间隔**统一由下列五条 AT 完成。  
> 汇总逻辑集中在 `lib/host_event.lua`，不维护平行状态机。  
> 双端开关：`WITH_T31X_HOSTEVT_SLEEP`（T31x）↔ `HOST_EVT_ENABLE`（4G）。

**版本**：v1.7 · 2026-06-26

> **电量 >20% 常电（v1.7）**：T31x 30s 空闲轮询时，即使 `HOSTEVT?` 无事件也 **不发** `HOSTIDLE=1`，先 `AT+GETCFG` 读 `battery=`。详见 [LOW_POWER_ENTER_STRATEGY.md §3](../power/LOW_POWER_ENTER_STRATEGY.md#3-ipc-侧电量-20-禁止-hostidle双端防护)。
...
| AT | 响应 / 行为 | 说明 |
|----|-------------|------|
| **`AT+HOSTEVT?`** | `+HOSTEVT:has_event,pending,types,sid,evt` + **media 字段** | **查询**待处理业务 + T31x 媒体分发参数；**仅读，不清除** |
| **`AT+HOSTEVTCLR`** | `+HOSTEVTCLR:OK` | **清除**可消费标记：GPIO pending + PIR `last/last_ts` |
| **`AT+HOSTIDLE=1`** | `+HOSTIDLE:OK` / `BUSY` / `ERROR` | `has_event=0` 时 T31x 请求 4G 对 T31x `enterSleep` |
| **`AT+HOSTIDLE?`** | `+HOSTIDLE:lowpower=%d` | 查询 4G 侧 `low_power_mode` 标志 |
| **`AT+HOSTEVTPOLL?`** | `+HOSTEVTPOLL:<ms>` | 查询 T31x 空闲轮询 `HOSTEVT?` 间隔（**毫秒**） |
...
T31x 在 **GPIO 等待超时、空闲休眠轮询** 时，按本间隔周期性发 `AT+HOSTEVT?`（此前 T31x 侧硬编码约 1s；现由 4G 侧配置下发）。

```text
T31x → 4G: AT+HOSTEVTPOLL?
4G → T31x: +HOSTEVTPOLL:30000 OK

T31x → 4G: AT+HOSTEVTPOLL=30000
4G → T31x: +HOSTEVTPOLL:OK OK
```
...
| T31x 侧 | 须在 `host_event.c` 读 `HOSTEVTPOLL?` 并应用间隔（4G 仅存储与应答） |
...
| 来源 | 行为 |
|------|------|
| `net_mqtt.hasHostQueue()` | 2006/2007 在 T31x 休眠时入队；云端 2011 停录待 T31x 同步 |
| `types_mask` bit3 | `config.lua` 默认 `0x0F` 含 mqtt |
| T31x 空闲轮询 | `pending=mqtt` **仅 block 休眠**，不触发 `media_dispatch`（4G `drainHostQueue` 在 T31x 就绪后处理） |
...
**T31x 侧（2026-06-26）**：`has_event=0` 且非 USB 时，先 `AT+GETCFG` 读 `battery=`；**>20% 不发** `HOSTIDLE=1`（IPC 保持常电）。仅 **≤20%** 或电量读数未知时才发 `AT+HOSTIDLE=1`。实现：`app/cat1/host_event.c`。

**4G 侧**（收到 `AT+HOSTIDLE=1` 时）：

1. `battery_guard.shouldHostSleep()==false`（>20% 常电）→ `+HOSTIDLE:BUSY`
2. 内部调用与 `AT+HOSTEVT?` 同源的 `build_hostevt_body()`
3. `has_event=1` → `+HOSTIDLE:BUSY`（拒绝断电）
4. `has_event=0` 且允许 → 异步 `t31x_ctrl.enterSleep({ reason="host_idle" })` → `+HOSTIDLE:OK`
...
| `+HOSTIDLE:USB` | USB 已插入（`HOST_USB_CFG`）；T31x 不得发 `HOSTIDLE=1` |
| `+HOSTIDLE:ERROR` | `t31x_ctrl` 不可用 |
...
USB 策略详见 [T31X_LOW_POWER.md §2.1](../power/T31X_LOW_POWER.md)。
...
| 说法 | 对应 AT | 含义 |
|------|---------|------|
| **精简** | `AT+HOSTEVT?` | 只返回 T31x **做决策**需要的字段：有没有待办、什么类型、怎么拍/录 |
...
|----|------|------|-----------------|
| `AT+HOSTEVT?` | **精简** | 汇总 wake/pir/record/mqtt → `has_event` + media | T31x **休眠/唤醒/媒体**唯一主入口 |
...
```text
   ...
   T31x 主路径                   cnt_* / 冷却 / has_work       buildHostEvtBody
          │                           │
          ▼                           ▼
...
| **不 dispatch** | T31x 空闲轮询**不得**调 `media_dispatch`（否则会误停录） |
| **消费** | 停录由二次 PIR / timer / `AT+RECORD=0` 等既有路径处理 |

T31x：`host_work_skip_idle_dispatch()`；4G：`host_event.isDispatchable()`。
...
- `has_event=1` → 跳过 T31x 断电（`shouldBlockT31xSleep`）
...
| 4G | `HOST_EVT_CFG.block_t31x_sleep_when_pending` | `true` |
| T31x | `build/config.mk` → `WITH_T31X_HOSTEVT_SLEEP=yes` | 开 |
| T31x | `syscfg [cat1] hostevt_sleep_enable=1` | 开 |
...
| 事件汇总 | `lib/host_event.lua` | `summarize()`、`shouldBlockT31xSleep()` |
| PIR 可消费标记 | `pir_ctrl.lua` | `clearConsumableMarkers()` |
| 休眠门禁 | `user/t31x_ctrl.lua` | `enterSleep()` |
...
| T31x 清除 | `AT+PIRCLR` | `AT+HOSTEVTCLR` |
...
- [ ] `has_event=0` 且 **battery ≤20%** 时 `AT+HOSTIDLE=1` → T31x 断电
...
- [ ] 4G 主动 `enterSleep` 在 `has_event=1` 时不断 T31x 电
...
| 文档 | 说明 |
|------|------|
| [T31X_HOSTEVT_PROTOCOL.md](T31X_HOSTEVT_PROTOCOL.md) | GPIO29→PB27 脉冲时序 |
| [T31X_IPC_4G_INTERACTION.md](T31X_IPC_4G_INTERACTION.md) | T31x↔4G 端到端流程与优化记录 |
| [T31X_LOW_POWER.md](../power/T31X_LOW_POWER.md) | rest / MQTT 1002/1003 |
| [UART_AT_COMMANDS.md](../mqtt/UART_AT_COMMANDS.md) | 全量 AT 清单 |
| [PIR_COOLDOWN_AND_COUNT.md](../pir/PIR_COOLDOWN_AND_COUNT.md) | PIR 计数 vs HOSTEVT 消费 |
