...
> T31x 主程序（`app/cat1/`）与 Air780 固件（`user/` + `lib/`）的端到端协作说明。  
> 相关专篇：[T31X_HOSTEVT_SLEEP.md](T31X_HOSTEVT_SLEEP.md)、[T31X_LOW_POWER.md](../power/T31X_LOW_POWER.md)、[T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)
...
│      t31x_ctrl (GPIO22 供电, GPIO29 唤醒脉冲)                     │
└────────────────────────────┬────────────────────────────────────┘
         GPIO29 脉冲          │  AT 双向
┌────────────────────────────▼────────────────────────────────────┐
│  T31x  serial.c / api.c / runtime.c / uart_host_cmd.c           │
...
| 通道 | 方向 | 典型内容 |
|------|------|----------|
| UART AT | T31x→4G | `HOSTEVT?` `PIRSTAT?` `RECORD=` `MQTTCFG` … |
| UART AT | 4G→T31x | `TIMESET` `PLAYSOUND` `IPCPOWEROFF` `+CAT1:MQTT` … |
| GPIO | 4G→T31x | 低脉冲 → PB27 下降沿 |
| MQTT | 4G↔云 | 1001/1002/1003、1010/1011 |

**编译真源**：T31x → `ipc_device_gb28181/app/cat1/`；4G → `/mnt/share/user/`、`/mnt/share/lib/`。
...
| 分支 | T31x `media_dispatch` | 4G |
|------|----------------------|-----|
| `action=photo` | JPEG 抓拍 | 1010 |
| `action=video/both` | MP4 开录 | 1010 + `t31x_active` |
| `recording=1`（二次 PIR） | `record_stop(pir_retrigger)` | `requestT31xStopRecord` |
...
详见 [T31X_HOSTEVT_SLEEP.md](T31X_HOSTEVT_SLEEP.md)。
...
- **不触发** T31x 空闲轮询 `media_dispatch`（避免误走 `recording=1` 停录分支）
...
sequenceDiagram
    participant A as app.lua
    participant G as host_uart/t31x_ctrl
    participant T as T31x runtime
...
GPIO27 **USB 插入**时，4G 与 T31x **互斥**低功耗指令（详见 [T31X_USB_HOSTIDLE.md](../power/T31X_USB_HOSTIDLE.md)）：
...
配置：`user/config.lua` → `HOST_USB_CFG`（`block_4g_rest_when_usb` / `notify_t31x_usb_state`）。

---

## 6. 双端开关对照

| 能力 | T31x `build/config.mk` | 4G `config.lua` |
|------|----------------------|-----------------|
...
| 优雅关机 | `WITH_T31X_LOW_POWER=yes` | `graceful_ipc=true` |
...
| 4G→T31x Host AT | `app/cat1/uart_host_cmd.c` |
...
| **v1.5** | **+CAT1:MQTT** | T31x 驱动 NET_STAT_LED（`notify_t31x_net_led`） |
| **v1.5** | **2011 云端停录** | `requestT31xStopRecord` 唤醒 T31x 同步停录 |
...
- [ ] 无 USB 冷启动 → T31x rest（charge 模块开启时亦如此）
...
# 管道
cat /tmp/ipc/cat1_uart.log | ./tests/cat1_record_sleep_log_check.sh -
```
...
| 文档 | 说明 |
|------|------|
| [T31X_IPC_CAT1_COMM_COMPLETENESS.md](T31X_IPC_CAT1_COMM_COMPLETENESS.md) | **T31x↔4G 通讯完善度分析**（双向 AT、时序闭环、缺口） |
| [T31X_USB_HOSTIDLE.md](../power/T31X_USB_HOSTIDLE.md) | **USB 插入 ↔ T31x/4G 低功耗互斥**（780EHM_PJ） |
| [T31X_HOSTEVT_SLEEP.md](T31X_HOSTEVT_SLEEP.md) | HOSTEVT 四条 AT |
| [T31X_HOSTEVT_PROTOCOL.md](T31X_HOSTEVT_PROTOCOL.md) | GPIO 脉冲时序 |
| [T31X_LOW_POWER.md](../power/T31X_LOW_POWER.md) | rest / MQTT 100x |
| [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md) | 录像 MQTT 1010/1011 |
| T31x [ipc_780EHM_PJ_interaction_analysis.md](../../../ipc_device_gb28181/docs/ipc_780EHM_PJ_interaction_analysis.md) | T31x 侧索引 |
