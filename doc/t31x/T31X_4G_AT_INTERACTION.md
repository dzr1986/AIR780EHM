# T31x Linux ↔ Air780 4G：AT 交互与 PIR 状态查询

> 先读框架简图：[T31X_4G_FRAMEWORK.md](T31X_4G_FRAMEWORK.md)  
> **AT 规范（MQTT + TCP）**：[T31X_CAT1_AT_COMMAND_SPEC.md](T31X_CAT1_AT_COMMAND_SPEC.md)  
> **MQTT 远程控制（帧率/录像/人形）**：[MQTT_CLOUD_REMOTE_CTRL_FLOW.md](../mqtt/MQTT_CLOUD_REMOTE_CTRL_FLOW.md)  
> 物理：UART（默认 115200 8N1，`\r\n` 行协议）  
> 4G 实现：`user/host_uart.lua`  
> T31x 实现：`t3x_linux/api.c`  
> 唤醒 GPIO：见 [T31X_HOSTEVT_PROTOCOL.md](T31X_HOSTEVT_PROTOCOL.md)
...
| 侧 | 职责 |
|----|------|
| **T31x** | TCP 业务通道参数、MQTT Broker、查询 4G 运行态/PIR 统计、收到 GPIO 脉冲后读 `HOSTEVT` |
| **4G** | 蜂窝/MQTT、PIR 硬件与策略、录像会话、冷却节流、**保存 PIR 判断计数**、主动 GPIO 唤醒 T31x |
...
> `SERVCREATE` 与 MQTT 独立：前者为 T31x 侧 TCP 长连接模板（当前 4G 脚本主要存配置）；MQTT 由 `net_mqtt.lua` 连接 Broker。
...
| `AT+HOSTEVT?` | `+HOSTEVT:` | `sid,evt`（查询 pending，`AT+HOSTEVTCLR` 清除，见 §4） |
| `AT+PIRSTAT?` | `+PIRSTAT:` | PIR 策略 + 计数 + 最近一次事件（§5） |
| `AT+PIRCLR` | `+PIRCLR:` | 清零 PIR 统计计数（不清配置） |
| `AT+TIME?` | `+TIME:` | Unix 秒；4G SNTP 未就绪时 `+TIME:0`（见 [TIME_SYNC.md](../overview/TIME_SYNC.md)） |

T31x API：`client_get_runtime_config`、`client_query_host_evt`、`client_get_pir_stat`、`client_sync_time_from_cat1`。
...
## 3. 4G → T31xx：GPIO 唤醒 + HOSTEVT

除 GPIO 脉冲外，4G 还可 **主动下发** AT（T31x `uart_host_cmd.c` 解析）：

| 命令 | 作用 | 文档 |
|------|------|------|
| `AT+PLAYSOUND=boot\|shutdown` | 提示音 | [BOOT_SHUTDOWN_SOUND.md](../pir/BOOT_SHUTDOWN_SOUND.md) |
| `AT+TIMESET=<unix>` | 设置系统时间 | [TIME_SYNC.md](../overview/TIME_SYNC.md) |
| `AT+FRAMERATE?` / `AT+FRAMERATE=` | MQTT **2024/2025** 帧率 | [MQTT_CLOUD_REMOTE_CTRL_FLOW.md §3](../mqtt/MQTT_CLOUD_REMOTE_CTRL_FLOW.md#3-帧率2024--2025) |
| `AT+RECORDCTRL=1/0` | MQTT **2012/2011** 直连开停录（T31x 在线） | [MQTT_CLOUD_REMOTE_CTRL_FLOW.md §4](../mqtt/MQTT_CLOUD_REMOTE_CTRL_FLOW.md#4-录像启停2011--2012) |
| `AT+PERSONDET?` / `AT+PERSONDET=` | MQTT **2026/2027** 人形检测 | [MQTT_CLOUD_REMOTE_CTRL_FLOW.md §5](../mqtt/MQTT_CLOUD_REMOTE_CTRL_FLOW.md#5-人形检测2026--2027) |
| `AT+VENC?` / `AT+VENCSET` 等 | MQTT **2020/2021** 编码 | [REMOTE_ENCODE_CONFIG.md](../mqtt/REMOTE_ENCODE_CONFIG.md) |

完整 4G→T31x 表见 [UART_AT_COMMANDS.md §3](../mqtt/UART_AT_COMMANDS.md#3-cat1--t31x4g-主动发t31x-答)。

4G 调用 `host_uart.ntfHost(sid, evt)` → GPIO29 低脉冲 → T31x `PB27` 下降沿。
...
T31x 流程：`gpio` 中断 → `AT+HOSTEVT?` → `client_handle_event` → 业务回调或重建 `SERVCREATE` + `MQTTCFG`。

**注意**：`HOSTEVT` 为 **单次 pending**，查询后清除；与 PIR 统计（累加计数）不同。

---

## 4. PIR 在 4G 上的分层与「各种情况」

PIR 判断**可以且应当保存在 4G**（传感器接在 Cat.1 GPIO30）。T31x 通过 `AT+PIRSTAT?` 读取，无需在 T31x 再实现一套冷却/录像状态机。

### 4.1 各分支判定与 4G 侧持久化

**结论：PIR 各分支的「是否触发 / 为何忽略」应保存在 4G**（RAM 计数 + 当前会话快照），T31x 用 `AT+PIRSTAT?` 读取，不必在 T31x 复刻冷却与录像状态机。

| 阶段 | 条件 | 4G 动作 | `last=` | 累加计数 |
|------|------|---------|---------|----------|
| 硬件 | T31x 烧录模式 | 忽略中断 | `ignore_burn` | `cnt_hw_ignore_burn` |
...
| 业务 | 正常检测 | 拍照/录像 + 唤醒 T31x | `detected` | `cnt_biz_detected`, `cnt_biz_photo`/`cnt_biz_video` |
...
GPIO30 中断 (pir_ctrl.lua)
  ├─ T31x 烧录模式 active        → cnt_hw_ignore_burn, last=ignore_burn
...
pir_ctrl.onPirTriggered
  ├─ suspended (T31x 烧录等)     → cnt_biz_ignore_suspend, last=ignore_suspend
  ├─ 录像中且 stopOnSecondPir   → cnt_biz_retrigger, 停录, last=retrigger
  └─ 正常检测                   → cnt_biz_detected, last=detected
        ├─ action=photo/both    → cnt_biz_photo, 唤醒 T31x(evt=0), MQTT 1010
        └─ action=video/both    → cnt_biz_video, 录像会话, 唤醒 T31x
录像停止
...
| 通道 | 用途 |
|------|------|
| **UART `AT+PIRSTAT?`** | T31x 本地策略、UI、日志、与 evt 无关的**累积统计** |
...
| `pending_wake` / `pending_sid` / `pending_evt` | 尚未被 `HOSTEVT?` 读走的唤醒（与 §3 一致） |
...
若 T31x 需「远程改 PIR 冷却 3s→30s」，需扩展 `AT+SETCFG=pir_cooldown,<ms>`（当前未实现，仍改 `config.lua` 烧录）。
...
| 文档 | 内容 |
|------|------|
| [UART_PROTOCOL.md](../mqtt/UART_PROTOCOL.md) | AT 简表 |
| [HOST_MQTT_UART.md](../mqtt/HOST_MQTT_UART.md) | MQTTCFG |
| [T31X_HOSTEVT_PROTOCOL.md](T31X_HOSTEVT_PROTOCOL.md) | GPIO 唤醒 |
| [PIR_HARDWARE.md](../hardware/PIR_HARDWARE.md) | 硬件 |
| [PIR_TRIGGER_INTERVAL.md](../pir/PIR_TRIGGER_INTERVAL.md) | 冷却间隔 |
| [PIR_PROTOCOL.md](../pir/PIR_PROTOCOL.md) | MQTT 2010/1010 |
