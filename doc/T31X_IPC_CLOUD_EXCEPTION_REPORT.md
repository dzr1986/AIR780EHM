...
> **工程**：780EHM_PJ（Cat.1）+ T31x IPC（`ipc_device_gb28181`）  
> **目的**：梳理 IPC 联网进程可能产生的异常，以及哪些已上报服务器、哪些仍仅本地日志。  
> **关联**：[MQTT_PROTOCOL.md](./MQTT_PROTOCOL.md) · [T31X_RECORD_MQTT_FLOW.md](./T31X_RECORD_MQTT_FLOW.md) · [MQTT_CLOUD_REMOTE_CTRL_FLOW.md](./MQTT_CLOUD_REMOTE_CTRL_FLOW.md) · **[T31X_IPC_EXCEPTION_MQTT_UPLINK.md](./T31X_IPC_EXCEPTION_MQTT_UPLINK.md)** · **[T31X_IPC_CAT1_SUPERVISION.md](./T31X_IPC_CAT1_SUPERVISION.md)**  
...
IPC（T31x Linux）**不直连 MQTT**。联网上报走两条独立链路：
...
| ----------- | -------- | ---------- | ----------- |
| **Cat.1**   | ✅        | ❌          | ✅           |
| **T31x IPC** | ❌        | ✅ MP4/JPEG | ❌（经串口通知 4G） |


```mermaid
flowchart TB
    subgraph IPC["T31x IPC"]
        REC[录像/抓拍]
        PD[人形 IVS]
        TF[TF 卡]
        ENC[编码/远控]
...
```text
1. 现象采集
   ├─ T31x 本地日志（CAT1 / HOST / PIR_SYNC / NET）
...
4. 平台侧核对
   ├─ 1011 reason 与 T31x TF 卡文件 / 日志是否一致
   ├─ 1003 IPCSTAT 与 AT+RECORD? / AT+IPCSTAT? 是否一致
   ├─ GB28181 离线时 1003.gb28181Online 是否镜像为 0
   └─ **恢复态**：异常后是否在下一条 **1003** 中字段回正（见 [T31X_IPC_EXCEPTION_MQTT_UPLINK.md §5](./T31X_IPC_EXCEPTION_MQTT_UPLINK.md)）
```
...
**§4.3 状态漂移专项流程**（4G `recording=1` 与 T31x 不一致）：

```text
T31x 停录/通知失败
  → IPC：IPCALERT / AT+RECORD=0 / 重试
  → 4G：pubIpcAlert → syncStopFromT31x + 1011
...
| IPC 侧事件       | 上报方式                                  | 云侧可见                                       |
| ------------- | ------------------------------------- | ------------------------------------------ |
| 首个 I 帧写盘成功    | `AT+RECORD=1`                         | **1010** `t31x_active` / **1012**           |
| 正常停录          | `AT+RECORD=0,reason=done`             | **1011** `source=t31x`                      |
...
| 平台/本地停录       | `reason=cloud` / `manual` / `timer` 等 | **1011**（`source=4g` 或 `t31x`）              |
...
| 直连开停录（T31x 在线） | `AT+RECORDCTRL=1/0`                 | 2012/2011 成功后 4G 主动发      |
...
| 场景                     | 行为                                     | 影响                                                |
| ---------------------- | -------------------------------------- | ------------------------------------------------- |
| `**no_person`**        | 仍**跳过** `AT+RECORD=0`；改 `IPCALERT no_person` → 4G 1011 + 清会话（§4.3） | 已修复 4G/T31x 漂移 |
...
```c
/* record_notify.c — no_person 改 IPCALERT，4G 侧 syncStopFromT31x + 1011 */
...
| P1  | 4G USB 网卡恢复失败    | T31x `AT+USBRECOVERY=EXHAUSTED` → 4G **1003** `usbRecovery*`；T31x 补 `IPCALERT usb_recovery_fail` |
...
| 优先级 | 异常                  | 修复措施 |
| --- | ------------------- | -------- |
| P1  | defer 宽限内始终无人       | `IPCALERT no_person` → 4G 1011 + `syncStopFromT31x`；GB Unconfirmed 仍并行 |
...
| 优先级 | 异常                  | 修复措施 |
| --- | ------------------- | -------- |
| P1  | RECORDCTRL 开录失败     | T31x `IPCALERT recordctrl_fail`；4G 2012 失败后 `pubIpcAlert` |
| P2  | 帧率/码率 ini 写成功运行时未生效 | **1025/1021** 增加 `runtimeApply`（`SetFramerate` / `SetVideoBitrate` 结果） |
| P2  | IPCPOWEROFF:BUSY    | T31x `IPCALERT ipcpoweroff_busy` → **1004** ipc_alert |
...
> **MQTT 后台具体 JSON / Topic / 恢复行为** → 专文 [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](./T31X_IPC_EXCEPTION_MQTT_UPLINK.md)
...
| **结果型** | **1010/1011** `/event` | 录像会话开始/结束及 reason | 1011 后会话结束；1003 `recordingT31x=0` |
...
**1003 IPCSTAT 字段（T31x → 4G → MQTT）**：

| 字段 | 含义 |
| --- | --- |
| `ipcReady` | T31x 生命周期 ready 且 Cat.1 链路正常 |
...
| `timeSynced` | 系统时间有效 |
| `recordingT31x` | 真实写盘 running/active |
| `usbRecovery*` | 4G 侧 USB 恢复（非 IPCSTAT，在 1003 主字段） |

**1004 ipc_alert 事件码（T31x → Cat.1）**：
...
| 字段 | 含义 |
| --- | --- |
| `ipcReady` | T31x 已上电且 runtime 正常（Cat.1 启用时含链路） |
...
| `timeSynced` | 系统时间有效 |
| `recordingT31x` | T31x 真实写盘 |

### 6.3 事件型（1004 ipc_alert）

T31x `AT+IPCALERT=code[,detail]` → Cat.1 `pubIpcAlert` → **1004** `action=ipc_alert`。
...
PIR / 2012 开录
  ...
  → IPC: record_start → MP4 线程
       ├─ 成功: AT+RECORD=1 → 1010 t31x_active
       └─ 失败: AT+RECORD=0,reason=* → 1011 source=t31x
...
详见 [T31X_RECORD_MQTT_FLOW.md](./T31X_RECORD_MQTT_FLOW.md) · IPC 仓库 `docs/cat1_record_status_sync.md`。
...
当前工程已有 **`ipc_supervision` 统一 T31x→4G 事件出口**（`ipc_cloud_report.h` 仅为兼容别名）；联调应 **MQTT + GB28181 双链对照**。
...
| IPC `docs/T31X_IPC_CAT1_COMM_COMPLETENESS.md`    | IPC↔4G 通讯完善度        |
...
| 文档 | 内容 |
| --- | --- |
| [T31X_IPC_ALERT_CODE_INDEX.md](./T31X_IPC_ALERT_CODE_INDEX.md) | **alertCode 逐条源码行号速查表** |
| [T31X_IPC_CAT1_SUPERVISION.md](./T31X_IPC_CAT1_SUPERVISION.md) | **Cat.1↔IPC 联合监督：架构、流程、源码索引、完善度** |
| [T31X_IPC_EXCEPTION_MQTT_UPLINK.md](./T31X_IPC_EXCEPTION_MQTT_UPLINK.md) | **MQTT 后台上行协议、JSON 示例、异常全表、恢复态说明** |
| [MQTT_PROTOCOL.md](./MQTT_PROTOCOL.md) | 200x/100x 编号与 Topic 约定 |
| [T31X_RECORD_MQTT_FLOW.md](./T31X_RECORD_MQTT_FLOW.md) | 1010/1011 录像会话时序 |
| [UART_AT_COMMANDS.md](./UART_AT_COMMANDS.md) | IPCSTAT / IPCALERT / RECORD? AT 格式 |

---

*IPC 仓库镜像：`docs/t31x_ipc_cloud_exception_report.md` · `docs/t31x_ipc_exception_mqtt_uplink.md` · `docs/t31x_ipc_cat1_supervision.md` · `docs/t31x_ipc_alert_code_index.md`*