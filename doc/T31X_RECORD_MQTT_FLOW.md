# T31x ↔ Cat.1 录像状态同步与 MQTT 上报流程

> 4G 工程摘要。IPC 权威全文：[`ipc_device_gb28181/docs/T31X_RECORD_MQTT_FLOW.md`](../../hif_ipcapp/ipc_device_org/ipc_device_gb28181/docs/T31X_RECORD_MQTT_FLOW.md)
...
|----------|------|----------|----------|
| **1001** | 4G→云 | PIR 唤醒 T31x（`uploadMode=auto`） | `uploadMode` |
| **1010** | 4G→云 | PIR 硬件触发 / 云端 query | `pirStatus=detected` |
| **1010** | 4G→云 | T31x 首个 I 帧写盘 | `pirStatus=t31x_active, active=1` |
| **1010** | 4G→云 | T31x JPEG 写入 SD | `pirStatus=snapshot_saved, snapshotPath=...` |
| **1011** | 4G→云 | 4G 定时/云端停录（T31x 未写盘时） | `source=4g, reason=timer\|...` |
| **1011** | 4G→云 | T31x `AT+RECORD=0` 结束/失败/二次 PIR | `source=t31x, reason=*` |
...
**原则**：4G 维护「录像会话策略」（`recording=1`、max_sec 定时）；T31x 维护「真实写盘结果」。两者必须通过 `AT+RECORD=` 同步，再由 4G 统一 MQTT 上报，避免平台只看到 4G 侧状态而不知道 SD 上是否有 MP4。

---

## 2. UART 双向通信

### 2.1 T31x → Cat.1（client_request）
...
### 2.3 Cat.1 本地 AT+RECORD?（T31x 主动问 4G）

T31x 发 `AT+RECORD?` 到 4G 模块时，4G `host_uart.lua` 返回 **4G 侧会话状态**：

```text
+RECORD:1,reason=active,active=1
```

与 Host 方向（4G 问 T31x）不同，注意区分收发方向。

---

## 3. PIR action 与唤醒次数

| `action`（PIRSTAT） | 4G 一次触发 | GPIO 唤醒 T31x | T31x `media_dispatch_wake_event` |
|---------------------|---------------|---------------|----------------------------------|
| `photo` | `PIR_WAKE_T31X` ×1 | ×1 | 仅 `media_snapshot` |
| `video` | `beginVideoSession` + `PIR_WAKE_T31X` ×1 | ×1 | 仅 `media_record_start` |
| `both` | `beginVideoSession` + `PIR_WAKE_T31X` ×1 | **×1** | **先 snapshot 再 record_start**（同周期） |
| `devinfo` | 无唤醒 | — | `client_report_devinfo`（4G 本地 MQTT） |

事件：`pir_ctrl.pubActionEvents` 只发布 **`APP_PIR_WAKE_T31X`**（不再分 `PIR_TAKE_PHOTO` + `PIR_RECORD_VIDEO` 两次唤醒）。
...
sequenceDiagram
    ...
    participant T31x as T31x media_ops
    ...
    C4G->>T31xx: GPIO 唤醒
    T31xx->>C4G: AT+PIRSTAT?
    C4G-->>T31xx: action=video,recording=0,max_sec=60
    T31xx->>REC: storage_mp4_start
    REC->>C4G: AT+RECORD=1（首个 I 帧）
    C4G->>MQTT: 1010 t31x_active
    alt 正常结束
