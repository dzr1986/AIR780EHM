...
> **关联**：[TIME_SYNC_FLOW.md](TIME_SYNC_FLOW.md)（同类 T31x 上电 + UART 命令模式）

---

## 1. 模块职责

经 UART 让 T31x 播放预置音效（`boot` / `shutdown` 等），用于冷启动、用户关机、低电关机等场景。
...
```text
等待 T31x 就绪：
  t31x_ctrl.powerOnWaitReady（HOST_IPC_CFG.boot_sound_wait_ready）
...
与 `time_sync` 类似，播放前需 T31x 上电；低电/ rest 场景由 `battery_guard` / `t31x_policy` 决定是否允许上电。
