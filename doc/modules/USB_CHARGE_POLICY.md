...
flowchart TD
    ...
    P1 --> APP[battery_guard.applyUsbPower]
...
## 4. USB 边沿编排（`battery_guard.applyUsbPower`）

真源：[`user/battery_guard.lua`](../../user/battery_guard.lua) §USB 边沿（与电量分档同模块；`app.start` 经 `start{…}` 注入 gpio/rndis/低功耗 hook，`GPIO_USB_DET_CHANGED` 仍订阅在 `app` EVNT_HNDL）。

| 边沿 | 行为 |
|------|------|
| **插入** | `runtime_power.setPowerStatus(1)` · `onUsbIns` · 退出 rest · `host_uart.pushUsbIdle(true)` · 取消 PWR 长按 |
| **拔出** | `power_status=0` · `pushUsbIdle(false)` · `onUsbRemove`（按电量重评估，高电量不进 rest） |

冷启动 `source=="boot"`：由 `bootPowerOn` 负责 T31 上电，避免与 `exitRest` 重复唤醒（见 [BATTERY_GUARD_TIERS.md](BATTERY_GUARD_TIERS.md)）。

**PMD 回退**：`MODULE_FLAGS.charge` 关闭时，`battery_guard.onPmdMessage` 用模组充电消息驱动 `applyUsbPower`。
