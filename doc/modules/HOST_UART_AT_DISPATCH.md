...
**版本**：2026-09-05（164：`+IPCPOWEROFF:BUSY` 见 §9 `join_busy`）
...
**跨族单源（架构 F/L2 条，2026-09-05）**：config 片段 `user/host.lua` `_G.HOST_PROTO_TMO` 六键：`ipcstat_query_ms`(2500)、`record_stop_ms`(22000)、`qry_default_ms`(3000)、`record_query_ms`(3500)、`t31x_power_wait_ms`(800)。host_uart `TMO_SHARED`、cloud/power/hostq、mqtt_dl_pir、time_sync/sound_prompt 缺省均引用此表。
...
- **断电与会话的仲裁（VERSION 161）**：① 策略触发的休眠（USB 拔出 / 电量 / `pir_watch_idle`，`skipPendingWorkCheck ~= true`）在任何会话进行中一律延后——`t31x_ctrl.blockSleep` 读 `host_uart.getUartSession()`，日志 `sleep_blocked_uart_session <name>`，与既有 `sleep_blocked_pending_work` 同一分支；② 用户显式 2002/AT 断电（`skipPendingWorkCheck`）不延后，但 `hostIpcPowerOff` 在其它会话进行中**有界等待**（≤ 本次 `poweroff_timeout_ms`，默认 90s，日志 `ipcpoweroff_rx wait_session`）其结束后再进入 poweroff 会话优雅断电；超时仍占用才回 false（`session_still_busy`）由 `gracePowOff` 硬切电。160 前二者都会在 500ms 内硬切电（写盘中拔卡）。
- **164 重入**：未见 STAGE 时收到 `+IPCPOWEROFF:BUSY` → 日志 `join_busy`，当作已有关机线程在跑，继续等 OK（或 90s），**不要**当立刻失败去硬切。已有 `uart_session=poweroff` 的第二路同样只等 OK，不把首包 STAGE 当完成。
