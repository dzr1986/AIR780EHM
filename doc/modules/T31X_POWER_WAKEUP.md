...
> **代码真源**：[`user/t31x_ctrl.lua`](../../user/t31x_ctrl.lua) · [`lib/t31x_policy.lua`](../../lib/t31x_policy.lua) · [`user/app.lua`](../../user/app.lua)  
> **策略详述**：[T31X_POLICY_GATE.md](T31X_POLICY_GATE.md) · **待处理业务**：[HOST_EVENT_PENDING.md](HOST_EVENT_PENDING.md)  
> **关联**：[T31X_HOSTEVT_SLEEP.md](../T31X_HOSTEVT_SLEEP.md) · [BOOT_SHUTDOWN_SOUND.md](../BOOT_SHUTDOWN_SOUND.md)
...
## 2. 唤醒门禁（`t31x_policy.mayPowerT31x`）

```mermaid
flowchart TD
    R[mayPowerT31x reason] --> P0{策略禁用/烧录?}
    P0 -->|通过| OK[允许]
    P0 -->|否| USB{USB 插入?}
    USB -->|是| OK
...
mayPowerT31x 通过
  ...
  → host_uart.ntfHost
       → powerOn（若未上电）
       → pulseMcuInt
  → 失败时 fallbackGpioWake（t31x_ctrl.wake）
```

**注意**：`onExitLowPower` 只调 `requestT31xWake`，不再重复 `onT31xWake`。

---

## 4. 休眠（`t31x_ctrl.enterSleep`）
...
```
t31x_ctrl.start()
  → t31x_policy.bootPowerOn
  → mayPowerT31x("boot")
  → powerOn（GPIO22 高）
```

冷启动 + USB 已插：`battery_guard.onUsbInserted({source="boot"})` **跳过** `wake_t31x`，避免双路上电。
...
| 来源 | reason | 路径 |
|------|--------|------|
| PIR 录像 | `pir_media` | app → requestT31xWake |
| 退出 rest | `exit_low_power` | onExitLowPower |
| USB 插入（非 boot） | `battery_usb` | wake_t31x hook |
...
- `t31x_policy.getDenyReason()` — 最近一次拒绝原因  
- `t31x_ctrl.getState()` — `powered_on` · `sleep_in_progress` · `last_action`
