...
> 框架上下文见 [T31_4G_FRAMEWORK.md](T31_4G_FRAMEWORK.md) §5。  
> 冷却间隔配置见 [PIR_TRIGGER_INTERVAL.md](PIR_TRIGGER_INTERVAL.md)。  
> AT 查询见 [T31_4G_AT_INTERACTION.md](T31_4G_AT_INTERACTION.md) §4～§5。
...
用于分析、日志、T31 侧决策参考，**不是**「当前是否在冷却」的实时开关。
...
| `cnt_hw_ignore_burn` | T31 烧录模式 active |
...
| | 冷却 | 计数 | WAKEVT |
|--|------|------|--------|
| **作用** | 实时决定本次中断是否进入业务 | 历史各分支累计多少次 | 这一次为何用 GPIO 唤醒 T31 |
| **生命周期** | 随时间自动过期（`cooldown_left_ms→0`） | 累加直到 `PIRCLR` 或重启 | 单次 pending，`WAKEVT?` 读后清除 |
| **主要查询** | `AT+PIRSTAT?` 中 `cooldown_left_ms` | `AT+PIRSTAT?` 中 `cnt_*` | `AT+WAKEVT?` |
...
**不必在 T31 再实现一套冷却时间戳或计数器**；传感器在 4G，状态保存在 4G，UART 查询即可。
...
- 二者都在 4G 的 `lib/pir` + `pir_runtime` 中维护，T31 用 `AT+PIRSTAT?` 读取。
