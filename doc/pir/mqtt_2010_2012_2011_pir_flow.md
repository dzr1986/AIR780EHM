...
> **术语**：实盘在 **TF 卡本地 MP4**，不是云端存片 → [T31X_RECORD_MQTT_FLOW.md §0](T31X_RECORD_MQTT_FLOW.md#0-术语不是云端录像)  
> **专题**：[mqtt_2012_1012_flow.md](mqtt_2012_1012_flow.md) · [mqtt_2011_1011_flow.md](mqtt_2011_1011_flow.md) · [MQTT_2011_T31X_STOP_EXPLAINED.md](MQTT_2011_T31X_STOP_EXPLAINED.md) · [PIR_PROTOCOL.md](../../../../mnt/share/doc/pir/PIR_PROTOCOL.md)
...
| **2012** | 平台 → 设备 | **平台指令开录**（TF 卡 MP4） | ✅ 受理即开 4G 会话并唤醒 T31x |
...
**2013** 是另一件事：平台要 **上传已有录像**，不是开/停写盘。见 [MQTT_2013_1013_UPLOAD_VIDEO.md](../mqtt/MQTT_2013_1013_UPLOAD_VIDEO.md)。`2010.uploadMode` 不能替代 2013。
...
PIR 硬件边沿              平台 MQTT
 ...
 onPirTriggered()     requestStartFromCloud()  ← 2012
      ...
      └──────────┬────────────┘
                 ▼
        pubActionEvents()
        beginVideoSession() + 唤醒 T31x + TF 卡写盘
                 │
                 ▼
        requestStopFromCloud()  ← 2011
        或定时器 / 二次 PIR / T31x AT+RECORD=0
```
...
- **共用**：同一 `pir_ctrl.session`、同一 `videoMaxDurationSec`、同一 T31x 唤醒/停录串口协议。
...
| rest / suspend | PIR 被忽略；2012 可能 `suspended`；低功耗见 [T31X_LOW_POWER.md](../../../780EHM_PJ/doc/power/T31X_LOW_POWER.md) |
...
| Cat.1 → 平台 | `/panshi/app/{IMEI}/pir` | **1010**（配置确认、PIR `detected`、`t31x_active`） |
...
| PIR 触发开录 | `1010` `detected`（`pir`）→ 可选 `1010` `t31x_active`（`pir`） |
| 2012 开录受理 | `1004` `action=pir_start,ret=0` + `1012`（`event`）→ 可选 `1010` `t31x_active` |
...
sequenceDiagram
    ...
    T->>T: record_stop
    T->>C: AT+RECORD=0
    C->>P: 1011 source=t31x 或 4g (event)
```
...
| T31x | 已上电或可被 4G GPIO 唤醒；TF 卡已插、可写 |
...
| 3 | 等待 3～10s | `pir`：可选 `1010` `t31x_active` | T31x 写盘确认 |
| 4 | 下发 **2011** | `event`：`1004` `pir_stop ret=0` + `1011` | T31x 在写盘时 1011 可能延迟最多约 15s |
| 5 | 查 TF 卡 | 新增 MP4 | 见 §9 |
...
| 有 1004 但 1011 很晚 | T31x 正在写盘 | 正常，最多约 15s 兜底 |
| 有 1012 无 MP4 | T31x 未上电/无卡/rest | **2002 exit** 给 T31 上电（不要用 2001）；查 TF 卡与 IPC 日志 |
...
全天录开机时 **一般不会** 向 Cat.1 发 `AT+RECORD=1`（无 PIR 会话标记），故 4G 侧 **`session.recording=0`**、`t31x_rec_active` 常为 0，但 TF 卡上文件仍在增长。
...
| 能力 | 模块 | 函数 |
|------|------|------|
| 2010 下行 | `net_mqtt.lua` | `dispatchDl2010` |
| 2012 下行 | `net_mqtt.lua` | `dispatchDl2012` |
| 2011 下行 | `net_mqtt.lua` | `dispatchDl2011` |
...
| T31x 协作 | `app.lua` | `onPirMediaAction` / `onPirStopRecording` |
...
| [MQTT_862323084068314.md](../mqtt/MQTT_862323084068314.md) | 本机 IMEI 速查表 |
