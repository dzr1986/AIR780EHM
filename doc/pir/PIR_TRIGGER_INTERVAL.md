...
> 配置索引：[`CONFIG.md`](../overview/CONFIG.md)  
> 硬件流程：[`PIR_HARDWARE.md`](../hardware/PIR_HARDWARE.md) · 协议：[`PIR_PROTOCOL.md`](PIR_PROTOCOL.md)  
> T31x 经 UART 读冷却丢弃次数：`AT+PIRSTAT?`（`cnt_hw_ignore_cooldown`），见 [`T31X_4G_AT_INTERACTION.md`](../t31x/T31X_4G_AT_INTERACTION.md)  
...
**硬件说明**：PIR 子板（FPC J1 → U1）无独立延时 IC，**`PIRMCU_DET` 直送 GPIO30**；间隔只能由软件配置，见 [`PIR_HARDWARE.md`](../hardware/PIR_HARDWARE.md) §1.1 原理图分析。
...
- [`PIR_HARDWARE.md`](../hardware/PIR_HARDWARE.md) — GPIO30、启动顺序、事件流  
