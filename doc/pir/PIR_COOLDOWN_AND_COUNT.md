...
> 框架上下文见 [T31X_4G_FRAMEWORK.md](../t31x/T31X_4G_FRAMEWORK.md) §5。  
> 冷却间隔配置见 [PIR_TRIGGER_INTERVAL.md](PIR_TRIGGER_INTERVAL.md)。  
> AT 查询见 [T31X_4G_AT_INTERACTION.md](../t31x/T31X_4G_AT_INTERACTION.md) §4～§5。
...
用于分析、日志、T31x 侧决策参考，**不是**「当前是否在冷却」的实时开关。
...
| `cnt_hw_ignore_burn` | T31x 烧录模式 active |
...
> **精简 vs 宽表**（`HOSTEVT?` / `PIRSTAT?` 分工、示例、能否合并）：见 [T31X_HOSTEVT_SLEEP.md §2](../t31x/T31X_HOSTEVT_SLEEP.md)。


| | 冷却 | 计数 | HOSTEVT |
|--|------|------|--------|
| **作用** | 实时决定本次中断是否进入业务 | 历史各分支累计多少次 | 这一次为何用 GPIO 唤醒 T31x |
...
**不必在 T31x 再实现一套冷却时间戳或计数器**；传感器在 4G，状态保存在 4G，UART 查询即可。
...
- 二者都在 4G 的 `pir_ctrl` + `pir_ctrl` 中维护，T31x 用 `AT+PIRSTAT?` 读取。
