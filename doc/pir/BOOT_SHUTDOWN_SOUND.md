# 4G + T31x 可视门铃：开机 / 关机提示音方案

> 适用：**Air780EHM（4G）+ T31x（协处理器）** 低功耗门铃  
> 硬件音频：T31x `HPOUT` / `SPEAK_EN`（见 [T31X_CAT1_GPIO.md](../hardware/T31X_CAT1_GPIO.md) §2.4）  
> 协作框架：[T31X_4G_FRAMEWORK.md](../t31x/T31X_4G_FRAMEWORK.md) · 唤醒：[T31X_HOSTEVT_PROTOCOL.md](../t31x/T31X_HOSTEVT_PROTOCOL.md) · 低功耗：[LOW_BATTERY_AND_LOW_POWER.md](../power/LOW_BATTERY_AND_LOW_POWER.md)

**当前固件状态（v1_20260529）**：4G `user/sound_prompt.lua` + T31x `audio_prompt.c` 已实现 `AT+PLAYSOUND`；**冷启动**在收到 T31x **首条 AT** 后由 4G 发 `AT+PLAYSOUND=boot`（非固定延时、非 T31x 自发）；低功耗休眠默认不播关机音；**PIR 唤醒不播开机音**（`boot_on_wake=false`）。详见 **§7.3**。
...
4G 侧无独立音频输出引脚；串口 **UART1** 与 T31x 通信（`config.UART_CFG`）。

### 2.2 电源与「开机 / 关机」在软件里的含义

本工程里 **T31x 可被单独断电**（GPIO22），4G 可常电联网，因此「开机音 / 关机音」要区分场景，不能简单等同「整机上电 / 整机掉电」。

```mermaid
flowchart TB
    subgraph on["类似开机"]
        A[整机上电] --> B[4G 启动 + GPIO22 上电 T31xx]
        C[退出低功耗] --> D[t31x_ctrl.wake 上电 + 脉冲]
        E[USB 插入恢复] --> D
    end
    subgraph off["类似关机"]
        F[业务低功耗] --> G[仅 GPIO22 断 T31xx]
...
| 场景 | 4G 模块 | T31x 电源 |
|------|---------|----------|
| 应用启动 | `t31x_ctrl.start()` → `powerOn()` | 上电 |
...
sequenceDiagram
    ...
    participant T as T31x Linux

  Note over T: 冷启动：4G 等 T31x 首条 AT 后发 AT+PLAYSOUND=boot
    U->>G: 长按关机
...
| 侧 | 职责 |
|----|------|
| **T31x** | Codec/`SPEAK_EN` 初始化；加载 `boot.wav` / `shutdown.wav`；执行播放；可选回 `+SOUNDACK` |
...
| 类型 | 原因 |
|------|------|
| **开机音** | T31x 上电后 Linux 启动耗时不定；4G 在 `boot_wait_host_ms` 内等 T31x **bootstrap 首条 AT**（如 `AT`）到达后再发 `AT+PLAYSOUND=boot`，避免 T31x 未就绪时盲目下发 |
...
| **USB 插入恢复 T31xx** | GPIO27 | ❌ 默认不播 | 充电/维护场景 |
| **业务休眠** | USB 拔出 / MQTT 2002 | ❌ 默认不播 | 仅断 T31x，非用户感知「关机」 |
| **用户长按关机** | PWRKEY | ✅ 关机音 | 明确反馈 |
| **云端关机** | MQTT 2004 `off` | ✅ 关机音 | 同用户关机 |
| **电量 ≤5% 自动关机** | `battery_guard` | ⚠️ 静音或极短 beep | 电量极低，避免长播导致来不及关机 |

插 **USB（GPIO27）** 时：`battery_guard` 忽略低电量并保持 T31x 上电（见 [LOW_BATTERY_AND_LOW_POWER.md](../power/LOW_BATTERY_AND_LOW_POWER.md)），**不因电量播关机音**。
...
| 命令 | 方向 | 说明 |
|------|------|------|
| `AT+PLAYSOUND=boot` | 4G → T31x | 播开机提示 |
| `AT+PLAYSOUND=shutdown` | 4G → T31x | 播关机提示 |
| `AT+PLAYSOUND?` | 查询 | `+PLAYSOUND:idle` / `playing` / `done` |
| `+SOUNDACK` | T31x → 4G | 播放结束（可选，用于精确等待） |

示例：

```text
4G → T31xx: AT+PLAYSOUND=shutdown
T31x → 4G: \r\nOK\r\n
... 播放中 ...
T31x → 4G: \r\n+SOUNDACK:shutdown\r\nOK\r\n   （可选）
```
...
| 文件 | 时机 | 动作 |
|------|------|------|
| `user/sound_prompt.lua` | `onAppStarted()` | 等 T31x 首条 AT（`HOST_UART_FIRST_AT`）→ 发 `AT+PLAYSOUND=boot` |
| `user/host_uart.lua` | 收到 T31x 首条 `AT*` | 置 `host_at_ready`，发布 `HOST_UART_FIRST_AT`（仅一次） |
| `user/app.lua` | `t31x_ctrl.start()` 之后 | 调用 `sound_prompt.onAppStarted()` |
...
| `user/t31x_ctrl.lua` | 可选 | `enterSleepWithSound()` 封装「通知 + 延时 + powerOff」 |
...
local function playShutdownIfNeeded()
    ...
    if not t31xModule.getState().powered_on then
        return
    end
...
sequenceDiagram
    participant A as app.lua
    participant T as t31x_ctrl

    A->>A: onEnterLowPower
    Note over A: shutdown_on_low_power=false
    A->>T: enterSleep → powerOff
    Note over T: T31x 立即断电，无提示音
```

### 7.3 冷启动开机音（4G 等 T31x 首条 AT）

整机上电后，4G **不会**在固定延时后盲目发 `AT+PLAYSOUND=boot`，而是等 T31x Linux bootstrap 发来**首条 AT**（通常为探测 `AT`）后再下发一次开机音。
...
| 超时仍未收到（T31x 未上电 / Linux 未起来） | 日志：`等待 T31x 首条 AT 超时，跳过开机音`；**不发** PLAYSOUND；等待任务结束 |
| 超时后 T31x 才上电发 AT | 4G 仍正常应答 AT；**本周期不再补播** 开机音 |

默认 `boot_wait_host_ms = 120000`（2 分钟），可在 `user/config.lua` → `SOUND_CFG` 调整。

#### 7.3.2 同一时段多条 AT：只发一次 PLAYSOUND

T31x 启动阶段可能连续发多条 AT（如 `AT`、`AT+GETCFG?` 等）。固件通过三层机制保证 **整次冷启动只下发一条** `AT+PLAYSOUND=boot`：
...
```text
T31x 连发:  AT → AT+XXX → AT+YYY   （同一启动窗口）
              ↓
...
| `battery_guard` ≤10% | 断 T31x + 1002，**不建议**长关机音 |
| `battery_guard` ≤5% | 整机 `pm.shutdown()`；可配置极短 beep 或静音 |
| USB 插入 | `onUsbInserted` 保持 T31x 上电，**不触发** 低电量关机音 |
| T31x 烧录模式 | 关停 MQTT/UART/RNDIS 时 **跳过** 提示音逻辑 |

---

## 9. 实施步骤（建议顺序）

1. **T31x**：单测本地播放 `boot.wav` / `shutdown.wav`（`SPEAK_EN` + Codec）。
2. **T31x**：串口实现 `AT+PLAYSOUND=` 与 `OK`（可选 `+SOUNDACK`）。
...
7. **文档**：在 [T31X_4G_AT_INTERACTION.md](../t31x/T31X_4G_AT_INTERACTION.md) 登记 `AT+PLAYSOUND`。
...
| # | 操作 | 预期 |
|---|------|------|
| 1 | 冷上电，T31x 正常启动 | T31x 发首条 `AT` 后，4G 发 **一条** `AT+PLAYSOUND=boot` |
| 1a | 冷上电，T31x 2 分钟内未发 AT | 超时跳过开机音，4G 其它功能正常 |
| 1b | T31x 启动连发多条 AT | 只播 **一次** 开机音 |
| 2 | PIR 唤醒录像 | **无** 开机音（默认） |
...
| 类型 | 路径 |
|------|------|
| GPIO / 音频引脚 | [T31X_CAT1_GPIO.md](../hardware/T31X_CAT1_GPIO.md) §2.4 |
| 唤醒与 AT | [T31X_HOSTEVT_PROTOCOL.md](../t31x/T31X_HOSTEVT_PROTOCOL.md)、[UART_PROTOCOL.md](../mqtt/UART_PROTOCOL.md) |
| T31x 媒体扩展 | [MEDIA_OPS.md](../../../t3x_linux/MEDIA_OPS.md) |
| 低功耗 / 电量 | [LOW_BATTERY_AND_LOW_POWER.md](../power/LOW_BATTERY_AND_LOW_POWER.md) |
