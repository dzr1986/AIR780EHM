# T3x → Cat.1 4G：AT 命令规范

> **真源**：T3x 侧 `t3x_linux/api.c` · 4G 侧 `user/host_uart.lua`  
> **配置**：`t3x_linux/client.ini` — `[channel]`（TCP）与 `[mqtt]`（MQTT）**独立两节**  
> 框架：[T3X_4G_FRAMEWORK.md](T3X_4G_FRAMEWORK.md) · 详表：[T3X_4G_AT_INTERACTION.md](T3X_4G_AT_INTERACTION.md)
...
| **T3x 统一下发** | 连接参数写在 `client.ini`，由 T3x `bootstrap` / 异常恢复时下发 4G |
...
与 **MQTT 无关**，描述 T3x 侧或 4G 侧要维护的 **TCP 长连接模板**（登录包、心跳、命中唤醒 hex 等）。
...
| 8 | `wake_hex` | hex 串 | 收到含此 pattern 的数据则唤醒 T3x |
...
| 失败 evt | 1（TCP 连接失败，预留） | 2（MQTT 离线 → GPIO 唤醒 T3x） |
...
| 层 | 文件 |
|----|------|
| T3x 组包/下发 | `t3x_linux/api.c` — `client_push_tcp_channel`, `client_push_mqtt_config`, `bootstrap` |
| T3x 配置读取 | `t3x_linux/config.c` — `[channel]` / `mqtt_*` |
| 4G AT 解析 | `user/host_uart.lua` — `uart_servcreate`, `uart_mqttcfg` |
| 4G TCP 执行 | `user/net_tcp.lua` — 与 T3x `client_push_tcp_channel` 对应 |
| 4G MQTT | `user/app.lua`, `user/net_mqtt.lua` |

---

## 10. T3x 与 4G 逻辑对照（必须一致）

| T3x 动作 | AT | 4G 模块 | 4G 行为 |
|----------|-----|---------|---------|
