...
平台通过 MQTT 下发 **2009** 格式化指令，Cat.1 经 UART 转发至 T31x IPC，完成 **停录 → umount → mkfs → mount → 可选重启** 后上报 **1009**。
...
sequenceDiagram
    ...
    UART->>T31x: 串口 AT
    T31x-->>UART: +RECORDCTRL:...
    Cat1->>UART: formatHostTfCard()
    UART->>T31x: AT+TFFORMAT=1,reboot=N
    T31x-->>UART: +TFFORMAT:STARTED
    Note over T31x: FIFO 停录 → 等 idle → umount → mkfs → mount
    T31x-->>UART: +TFFORMAT:OK,reboot=N
...
| `reboot` | `0` 不重启（默认）；`1` 格式化成功后 T31x 重启。未传时用 `HOST_TFCARD_FORMAT_CFG.reboot_after` |
...
| `not_recording` 等 | T31x 侧错误码字符串（见 §4.3） |
...
2. **`recordCtrlStop({ reason = "tfcard_format" })`**：`AT+RECORDCTRL=0,tfcard_format`，通知 T31x 停录。
...
| 方向 | 内容 |
|------|------|
| Cat.1 → T31x | `AT+TFFORMAT=1,reboot=0` |
| T31x → Cat.1 | `+TFFORMAT:STARTED`（已接受，后台执行） |
| T31x → Cat.1 | `+TFFORMAT:OK,reboot=0` 或 `+TFFORMAT:ERROR,ret=...` |
...
| T31x | `TFFORMAT start`、`fifo stop submitted`、`umount ok`、`mkfs ok`、`mount ok`、`TFFORMAT done` |
...
2. T31x 休眠时 2009 会入队 `pendingHostQueue` 并唤醒 T31x，与 2006/2007 相同。
