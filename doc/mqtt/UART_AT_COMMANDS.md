# T31x ↔ Cat.1 串口 AT 指令一览

> **物理**：UART 115200 8N1（`config.lua` → `UART_CFG`）  
> **代码真源**：T31x→4G `user/host_uart.lua` · 4G→T31x `cat1_host/uart_host_cmd.c`  
> **相关文档**：[UART_PROTOCOL.md](UART_PROTOCOL.md) · [T31X_CAT1_AT_COMMAND_SPEC.md](../t31x/T31X_CAT1_AT_COMMAND_SPEC.md) · [T31X_4G_AT_INTERACTION.md](../t31x/T31X_4G_AT_INTERACTION.md) · [MQTT_CLOUD_REMOTE_CTRL_FLOW.md](MQTT_CLOUD_REMOTE_CTRL_FLOW.md)（帧率/录像/人形）

---

## 1. 方向说明（先看这个）

本工程里 **Host = T31x Linux 协处理器**，**Cat.1 = Air780EHM 4G 模组**（跑 LuatOS）。
...
| 方向 | 谁发送 | 谁解析应答 | 典型用途 |
|------|--------|------------|----------|
| **T31x → Cat.1** | T31x `api.c` / 串口工具 | 4G `host_uart.uart_at_cmd` | 配置 MQTT/TCP、查状态、低功耗、OTA |
| **Cat.1 → T31x** | 4G `uart_bridge.sendString` | T31x `uart_host_cmd.c` | 白光灯、对时、提示音、MQTT 联网 URC |
| **Cat.1 → T31x（GPIO）** | 4G GPIO29 脉冲 | T31x PB27 中断 | 唤醒；原因用 `AT+HOSTEVT?` + `AT+HOSTEVTCLR` |

**约定**：每条命令一行，以 `\r\n` 结束；成功常见尾缀 `OK\r\n`，失败 `ERROR\r\n`。

> **注意**：部分旧文档写 `AT+GETCFG?`、`AT+PIRSTAT?`，当前 4G 固件为 **精确匹配**（见下表「4G 实际命令字」）。T31x 侧若带 `?`，需与 `host_uart.lua` 保持一致或改 T31x 发送串。

---

## 2. T31x → Cat.1（Host 发，4G 答）
...
| `AT+PIRSTAT` / `AT+PIRSTAT?` | `+PIRSTAT:` | **宽表**：PIR 策略、`cnt_*`、冷却 + 附带 `has_work`；运维/调试；见 [T31X_HOSTEVT_SLEEP.md §2](../t31x/T31X_HOSTEVT_SLEEP.md) |
| `AT+RECORD` / `AT+RECORD?` | `+RECORD:` | **4G 侧录像会话**（`recording,reason,active`）；T31x 上报见 §2.9 |
| `AT+PIRCLR` | `+PIRCLR:OK` | 清零 `cnt_*` 统计（**运维**；业务消费用 `HOSTEVTCLR`） |
| `AT+HOSTEVT` / `AT+HOSTEVT?` | `+HOSTEVT:` | **精简**：`has_event,pending,types,sid,evt` + media；T31x 休眠/唤醒主入口 |
| `AT+HOSTEVTCLR` | `+HOSTEVTCLR:OK` | 清除 pending 唤醒 + PIR 可消费 `last`（不清 `cnt_*`） |
| `AT+HOSTEVTPOLL` / `AT+HOSTEVTPOLL?` | `+HOSTEVTPOLL:<ms>` | T31x 空闲轮询 `HOSTEVT?` 间隔（**毫秒**）；默认 30000 |
| `AT+HOSTEVTPOLL=<ms>` | `+HOSTEVTPOLL:OK` | 设置轮询间隔；范围见 `HOST_EVT_CFG.poll_interval_*_ms` |
| `AT+HOSTIDLE?` / `AT+HOSTIDLE=0/1` | `+HOSTIDLE:` | 休眠门禁；见 [T31X_HOSTEVT_SLEEP.md](../t31x/T31X_HOSTEVT_SLEEP.md) |
...
| `AT+USBRESET` | `+USBRESET:OK` / `BUSY` / `DISABLED` / `REST` | **T31x → 4G**：mismatch 恢复，重绑 RNDIS 后再推 `+CAT1:USB,1`；`REST`=4G rest 且 T31x 已断电，不 rebind |
| `AT+USBRESET?` | `+USBRESET:busy,count,last,rest_blk,t31x_pwr` | 查询重枚举状态 |
| `AT+USBRECOVERY=OK,0` / `EXHAUSTED,N` | `+USBRECOVERY:OK` / `EXHAUSTED` | **T31x → 4G**：仅上报恢复结果（**不** rebind）；4G 写 MQTT 1003 |
| `AT+USBRECOVERYRESET` | `+USBRECOVERYRESET:OK` | **4G → T31x**：MQTT 2003 重置，清零 T31 侧重试计数 |

### 2.5 白光灯（T31x 也可经 4G 控制）

| 命令 | 响应 | 行为 |
|------|------|------|
| `AT+WLED=0/1` / `AT+WLEDEN=0/1` | `+WLED:n` `OK` | 4G 更新状态并 **转发** `AT+WLED=n` 到 T31x |
...
| `AT+SETCFG=hostevt_poll,<毫秒>` | T31x 空闲 `HOSTEVT?` 轮询间隔（同 `AT+HOSTEVTPOLL=`） |
...
```text
T31x → Cat.1: AT+PIRSTAT?
Cat.1 → T31x: +PIRSTAT:action=devinfo,recording=0,max_sec=0,... OK
```

T31x 应跳过拍照/录像。默认 **MQTT 2006** 由 Cat.1 自行查 GB28181 + 上报 **1006**，**不唤醒** T31x。

**`AT+IPCINFO?`**（T31x → Cat.1）：

```text
T31x → Cat.1: AT+IPCINFO?
Cat.1 → T31x: \r\n+IPCINFO:imei=862323084068124,gb28181Id=34020000001320000001\r\nOK\r\n
```

**`AT+MQTTPUB`**（T31x → Cat.1，4G 代发 MQTT）：

```text
T31x → Cat.1: AT+MQTTPUB=identity;{"deviceNo":"862...","dataType":"1006",...}
Cat.1 → T31x: \r\n+MQTTPUB:OK\r\n
```

失败：`\r\n+MQTTPUB:ERROR\r\n`（无第二行 `OK`/`ERROR`）。topic = `/panshi/app/{imei}/` + suffix（如 `identity`）。

**`AT+TFCARD?`**（Cat.1 → T31x，MQTT 2007 同源）：

```text
Cat.1 → T31x: AT+TFCARD?
...
T31x 挂载点：`client.ini` → `tf_mount_path`（默认 `/mnt/sd`）。无卡时 `present=0`，容量字段为 0。

未识别命令 → `\r\nERROR\r\n`（`AT+RIL=1` 时可能透传 modem）。

### 2.9 录像状态同步（T31x ↔ 4G）

**T31x → Cat.1**（`app/cat1/record_notify.c`，4G `host_uart.uart_record_notify`）：

```text
T31x → Cat.1: AT+RECORD=1
Cat.1 → T31x: +RECORD:1,active=1 OK

T31x → Cat.1: AT+RECORD=0,reason=done|time_sync|no_iframe|...
Cat.1 → T31x: +RECORD:0,reason=done OK
```

4G 收到后：`syncStopFromT31x()` 清 `recording`，发布 `APP_T31x_RECORD_*` → MQTT **1010** `t31x_active` / **1011** `source=t31x`。

**T31x 问 4G 本地会话**（`AT+RECORD?` 由 T31x `client_request` 发起）：

```text
+RECORD:1,reason=active,active=1
```

**Cat.1 问 T31x 真实写盘**（`host_uart.queryHostRecord()` → Host UART）：

```text
Cat.1 → T31x: AT+RECORD?
T31x → Cat.1: +RECORD:running=1,active=0,ch=0,reason=idle OK
```
...
详见 [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)。

---

## 3. Cat.1 → T31x（4G 主动发，T31x 答）

实现：`cat1_host/uart_host_cmd.c`。T31x 收到后以 `\r\n+XXX:...\r\nOK\r\n` 应答。

| 4G 下发命令 | T31x 响应 | 触发场景 |
|-------------|----------|----------|
| `AT+WLED=0/1` | `+WLED:n` `OK` | MQTT 2004 白光灯；或 T31x 经 4G 转发 |
| `AT+WLED?` / `AT+WLEDEN?` | `+WLED:n` `OK` | 4G 查询 T31x GPIO 侧（若 4G 主动发） |
| `AT+TIMESET=<unix>` | `+TIMESET:OK` `OK` | 唤醒前对时（`time_sync.lua`） |
| `AT+GB28181?` / `AT+GB28181` | `+GB28181:<id>` `OK` | 读 T31x GB28181 设备 ID（MQTT 1006） |
| `AT+TFCARD?` / `AT+TFCARD` | `+TFCARD:present,n,total_mb,used_mb,free_mb` `OK` | 读 T31x TF/SD 卡（MQTT 1007） |
| `AT+IPCSTATUS?` / `AT+IPCSTATUS` | `+IPCSTATUS:ready\|idle\|shutting_down` `OK` | T31x 生命周期（§3.4） |
| `AT+RECORD?` / `AT+RECORD` | `+RECORD:running=,active=,ch=,reason=` `OK` | T31x 真实录像状态（§2.9） |
...
| `AT+RECORDCTRL=1,<max_sec>` | `+RECORDCTRL:OK,1,max_sec=<n>` `OK` | T31x 在线时平台开录；MQTT **2012** 成功后 4G 主动发 |
| `AT+RECORDCTRL=0,<reason>` | `+RECORDCTRL:OK,0,reason=<text>` `OK` | T31x 在线时平台停录；MQTT **2011** 成功后 4G 主动发；reason 默认 `cloud` |
...
| `AT+UPLOADNEED=1,reason=person,type=1,start=,end=,alarmTs=,uploadTs=,file=,msgId=,pirStatus=`（T31x→4G） | `+UPLOADNEED:ok,need=1` | 人形入队：带 **文件名+时间窗**；4G 转 **1013** `reply=1 stage=queued videoType=1` |
| `AT+UPLOADRESULT=ret=,type=,start=,end=,uploadTs=,file=,httpPath=,msgId=,reason=,msg=`（T31x→4G） | `+UPLOADRESULT:ok,ret=` | HTTP 完成/失败；4G 转 **1013** `reply=0` `stage=uploaded\|fail` |
| `AT+UPLOADPROGRESS=pct=,sent=,total=,type=,msgId=,file=,stage=`（T31x→4G） | `+UPLOADPROGRESS:ok,pct=` | HTTP 进度；4G 立刻 ACK 再 **1013** `reply=1` `stage=uploading\|waiting_resp` `percent` |
| `AT+IPCSTAT?` | `+IPCSTAT:ipcReady=,gb28181Online=,tfPresent=,personDetectEnabled=,personDetectAvailable=,timeSynced=,recordingT31x=,cat1Link=` `OK` | §6.2 扩展状态；MQTT **1003** 周期携带 |
| `AT+IPCALERT=<code>[,<detail>]`（T31x→4G URC） | `+IPCALERT:OK` | §6.3 事件；4G 转 **1004** `action=ipc_alert` |
...
T31x **不识别** 上节 2 中的 `SERVCREATE/MQTTCFG/GETCFG` 等（那些只在 4G 解析）。

### 3.1 非 AT 的主动上行（URC）

| 行格式 | 方向 | 说明 |
|--------|------|------|
| `+CAT1:MQTT,0/1` | 4G→T31x | MQTT 离线/在线 → T31x `GpioNetStatSetCat1Mqtt`（需 `LED_CFG.notify_t31x_net_led=true`） |
| `+CAT1:USB,0/1` | 4G→T31x | USB 拔出/插入 → T31x 允许/禁止 `HOSTIDLE` 休眠轮询（`HOST_USB_CFG.notify_t31x_usb_state`） |
| `+RXHEX:<hex>` | 4G→T31x | 仅当 T31x 发了 `SETCFG hexrpt=1` 且对端有数据 |

### 3.2 GPIO 唤醒（非串口 AT）

4G `ntfHost(sid, evt)` → GPIO29 脉冲 → T31x 中断 → T31x 再发 **`AT+HOSTEVT?`** 读原因，处理后 **`AT+HOSTEVTCLR`**。
...
4G 在 `sound_prompt.onAppStarted()` 中等待 T31x **首条 AT**（事件 `HOST_UART_FIRST_AT`），收到后再发 **一条** `AT+PLAYSOUND=boot`：
...
- 同一启动窗口内 T31x 连发多条 AT 时，仅首条触发开机音；详见 [BOOT_SHUTDOWN_SOUND.md](../pir/BOOT_SHUTDOWN_SOUND.md) §7.3

### 3.4 T31x 电源（Cat.1 推荐流程）

Host AT 由 T31x 实现（产品：`gb28181_dev_exit()`、`sync()` 等；桩：`cat1_host/ipc_host.c`）。Cat.1 编排见 `t31x_ctrl.lua`。
...
```text
Cat.1 → T31x: AT+IPCSTATUS?     （无应答 / +IPCSTATUS:idle）
Cat.1: GPIO22 上电
Cat.1 轮询 AT+IPCSTATUS? 直到 +IPCSTATUS:ready
Cat.1 → T31x: AT+PLAYSOUND=boot （可选，sound_prompt 冷启动）
```
...
| 查 T31x GB28181 ID | `AT+GB28181?`（4G→T31x） | **2006**→**1006** |
| 查 T31x TF/SD 卡 | `AT+TFCARD?`（4G→T31x） | **2007**→**1007** |
| 查状态/电量/USB | `AT+GETCFG` 或 `2003`→1003 | `dataType=2003` |
| 低功耗 | `AT+LOWPOWER=` | `2002` |
| 重启/关机/OTA | `AT+REBOOT` 等 | `2004` |
| 白光灯 | `AT+WLED=` | `2004 action=wled` |
| PIR 配置 | — | `2010` |
| 平台开录 / 停录 | `AT+RECORDCTRL=1/0`（T31x 在线） | **2012** / **2011** |
| 请求上传视频 | `AT+UPLOADVIDEO=`（T31x 在线） | **2013** → **1013** |
| IPC 扩展状态 | `AT+IPCSTAT?` | **1003** 字段 ipcReady/gb28181Online/… |
| IPC 异常事件 | T31x 发 `AT+IPCALERT=` | **1004** `action=ipc_alert` |
...
| T31x→4G 协议 | `user/host_uart.lua` |
| 4G→T31x 协议 | `cat1_host/uart_host_cmd.c` |
| T31x 电源桩 | `cat1_host/ipc_host.c` |
| T31x 电源编排 | `t31x_ctrl.lua` |
| TF 卡（T31x） | `cat1_host/tf_card.c` |
...
| WLED GPIO（T31x） | `cat1_host/wled.c` |
| 对时 | `user/time_sync.lua` ↔ T31x `time_sync.c` |
| 提示音 | `user/sound_prompt.lua` ↔ T31x `audio_prompt.c` |
| 唤醒脉冲 | `user/host_uart.lua` → `ntfHost` + `t31x_ctrl` |

---

## 6. 调试示例

**T31x 侧（经 4G 串口工具连 Cat.1 UART）**
...
**验证 4G→T31x**（需 T31x 上电）：日志或抓包应出现 `AT+GB28181?`、`AT+TFCARD?`（MQTT 2006/2007 触发）；WLED 转发 `AT+WLED=1`。
