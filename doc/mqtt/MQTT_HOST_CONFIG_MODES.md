...
T31x Linux 与 Air780 4G 之间通过 UART `AT+MQTTCFG` 下发 MQTT Broker 参数。  
工程**当前采用思路 B**；思路 A 可作为无默认 Broker、强依赖 T31x 的备选方案。

相关实现：

- 串口协议：[HOST_MQTT_UART.md](HOST_MQTT_UART.md)
- T31x 侧：`t3x_linux/client.ini` `[mqtt]`、`api.c` `push_mqtt_config()`
- 4G 侧：`user/config.lua`、`user/host_uart.lua`、`user/app.lua`、`user/net_mqtt.lua`

---

## 思路 A：上电等 T31x，参数仅在 T31x 端
...
- `shouldDeferMqttUntilHost()`：`defer_boot` 且 `MODULE_FLAGS.t31x_app` 为真时，`bootMqtt` 不调用 `startMqtt`
- `on_mqtt_cfg`：收到 T31x 配置后再启动 MQTT
...
- 无 T31x 或串口未就绪时**永远不上 MQTT**
- 联调 4G 单机需临时改 `defer_boot=false` 并填写 `MQTT_CFG`

### 适用场景

- 量产板必接 T31x，Broker 只由主机配置
- 不希望 4G 侧出现任何默认服务器地址

---

## 思路 B：上电 4G 自动连，T31x 串口覆盖重连（当前方案）
...
| T31x 就绪后 | 可继续用默认连接 | `bootstrap` 发 `AT+MQTTCFG` |
...
- `bootMqtt`：不等待 T31x，蜂窝就绪即 `startMqtt`
- `on_mqtt_cfg`：`setMqttCfg` + 已启动则 `restart()`，未启动则 `startMqtt()`

### 优点

- **脱机可测**：不接 T31x 也能验证 4G MQTT
- T31x 晚启动不影响首连；主机仍可**动态改 Broker** 并强制重连
...
| 上电 MQTT | 否 | 是 |
| T31x `client.ini [mqtt]` | 唯一参数源 | 覆盖源（建议与默认一致） |
| T31x 下发后 | `startMqtt` | `restart()` 重连 |
...
2. 再接 T31x、`./main client.ini`：日志 `T31x 覆盖 MQTT`，若参数不同则重连新 Broker
3. 修改 `client.ini` 中 `mqtt_host` 后重启 T31xx：4G 应切到新地址

**思路 A（若启用）**

1. 仅 4G 上电：无 MQTT 连接，日志 `MQTT 等待 T31x AT+MQTTCFG`
2. T31x bootstrap 后：出现 `MQTTCFG`、`MQTT 已按 T31x 配置连接`
