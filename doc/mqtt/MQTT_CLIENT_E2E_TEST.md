...
> **工具链总报告**：[CAT1_TOOLCHAIN_TEST_REPORT.md](../_audit/CAT1_TOOLCHAIN_TEST_REPORT.md)（烧 Lua → 客户端收发 → 自动测试）  
...
**独立 exe**：`tools/build_mqtt_gui_exe.bat` 生成 `dist/PanshiMqttClient.exe`（无 Python 环境也可双击）。首次运行会在 exe 旁写出 `config.json` 和 `doc/mqtt/MQTT_PROTOCOL.md`。
