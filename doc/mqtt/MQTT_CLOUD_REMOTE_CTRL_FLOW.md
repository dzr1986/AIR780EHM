# MQTT 远程控制：帧率 / 录像 / 人形检测（Cat.1 → UART → T31x）

> **工程**：780EHM_PJ（Air780EHM + T31x）  
> **现网对本机 IMEI**：**`862323084068124`**（另一台样机 `…8314` 见 [MQTT_862323084068314.md](MQTT_862323084068314.md)，勿混用主题）  
> **代码真源**：`user/net_mqtt.lua` · `user/host_uart.lua` · T31x `uart_host_cmd.c` / `cloud_remote_ctrl.c`  
> **关联**：[MQTT_PROTOCOL.md](MQTT_PROTOCOL.md) · [UART_AT_COMMANDS.md](UART_AT_COMMANDS.md) · [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md) · [MQTT_2011_T31X_STOP_EXPLAINED.md](../pir/MQTT_2011_T31X_STOP_EXPLAINED.md) · [REMOTE_ENCODE_CONFIG.md](REMOTE_ENCODE_CONFIG.md) · **全指令实机**：[MQTT_ALL_CMD_FLOW_TEST.md](MQTT_ALL_CMD_FLOW_TEST.md)
...
user/net_mqtt.lua          ← 解析 dataType，组应答 JSON
    │
    ▼
user/host_uart.lua         ← 唤醒 T31x、发 UART AT、等 +XXX 应答
    │
    ▼ UART 115200
T31x uart_host_cmd.c
    │
    ▼
...
T31x **不直连 MQTT**；平台只与 Cat.1 通信。

---

## 2. 三条控制总览

| 业务 | MQTT 下行 | MQTT 上行 | 主题后缀 | 4G→T31x AT |
|------|-----------|-----------|----------|-----------|
...
> **录像**：T31x **休眠**时走 GPIO 唤醒 + `AT+HOSTEVT?`；**已在线**时 2011/2012 成功后额外发 `AT+RECORDCTRL` 加快开停。
...
**T31x**：`SetFramerate()` 热更新 + `save_framerate()` 写 `syscfg.ini`（无需整包 VENCSET 重启，除非改分辨率等）。

### 3.3 UART 帧率 AT

| 命令 | 方向 | 成功应答 |
|------|------|----------|
| `AT+FRAMERATE?` | 4G→T31x | 多行 `+FRAMERATE:<cam>,<stream>,<fps>` + `+FRAMERATE:END` + `OK` |
| `AT+FRAMERATE?0,0` | 4G→T31x | 指定 cam/stream |
| `AT+FRAMERATE=0,0,20` | 4G→T31x | `+FRAMERATE:OK,0,0,20` + `OK` |
...
2. app.onPirMediaAction → GPIO 唤醒 T31x（若断电）
3. T31x: AT+HOSTEVT? → media_dispatch_wake_event → TF MP4
4. 若 isT31xHostReady():
      host_uart.recordCtrlStart({ max_sec=90 })
      → AT+RECORDCTRL=1,90
5. T31x 写盘: AT+RECORD=1 → 4G → MQTT 1010 t31x_active
6. 结束: AT+RECORD=0 → MQTT 1011 source=t31x
```

### 4.2 停录 2011

白话逐步说明（两层录像、复位掉电、1004 vs 1011）：[MQTT_2011_T31X_STOP_EXPLAINED.md](../pir/MQTT_2011_T31X_STOP_EXPLAINED.md)
...
2. 若 T31x 在线: AT+RECORDCTRL=0,cloud
3. T31x 停写盘 → AT+RECORD=0 → 1011
```
...
| 命令 | 方向 | 说明 |
|------|------|------|
| `AT+RECORDCTRL=1,<max_sec>` | 4G→T31x | 直连开录；默认 max_sec=60 |
| `AT+RECORDCTRL=0,<reason>` | 4G→T31x | 直连停录；reason 默认 `cloud` |
| `AT+RECORD?` | 4G→T31x | 查 T31x 写盘状态 `running/active/ch/reason` |
| `AT+RECORD=1` | T31x→4G | 首 I 帧已写盘（原有） |
| `AT+RECORD=0,reason=*` | T31x→4G | 停录通知（原有） |
...
3. Cat.1 解析 2013；T31x 在线: AT+UPLOADVIDEO=1,2,<start>,<end>,<max_sec>,<messageId>
4. T31x clip_upload_request → HTTP type=2 回放（或 type=1 侦测）
5. MQTT 1013 reply=1（不发 1004）；文件随后出现在 7003 playback/
```

| 命令 | 方向 | 说明 |
|------|------|------|
| `AT+UPLOADVIDEO=<need>,<type>,<start>,<end>[,<max_sec>][,<msgid>]` | 4G→T31x | 按时间窗排队抽片上传 |
| `AT+UPLOADNEED=1,reason=…` | T31x→4G | 本端认为需要上传；4G 转 1013（无 reply） |
...
**运行时**：T31x 写 `[person_detect] enable` 到 ini；IVS 线程读 `person_detect.enable`。**关闭**立即停检；**开启**若启动时未 init IVS 需重启 T31x 才完整启用。
...
sequenceDiagram
    ...
    T->>H: AT+RECORD=1
    M->>P: 1010 t31x_active
    T->>H: AT+RECORD=0,reason=done
    M->>P: 1011 source=t31x
```
...
**版本**：v1.0 · 2026-06-26 · 与 `user/net_mqtt.lua` / `user/host_uart.lua` / T31x `cloud_remote_ctrl.c` 同步
