...
> **协议真源**：[`MQTT_PROTOCOL.md`](MQTT_PROTOCOL.md) · **烧录手册**：[`CAT1_FLASH_TOOL.md`](CAT1_FLASH_TOOL.md) · **联调步骤**：[`MQTT_CLIENT_E2E_TEST.md`](MQTT_CLIENT_E2E_TEST.md) · **全指令实机**：[MQTT_ALL_CMD_FLOW_TEST.md](MQTT_ALL_CMD_FLOW_TEST.md)
...
另一台样机 `862323084068314` 文档见 [`MQTT_862323084068314.md`](MQTT_862323084068314.md)，主题不可混用。
...
sequenceDiagram
    ...
    alt 仅 4G（2001/2003/2005/2008…）
        Cat->>Br: 100x 秒回
    else 需 T3x（2006/2007/2020–2031…）
        Cat->>T3: 唤醒 + UART AT
        T3-->>Cat: 应答
        Cat->>Br: 100x / 102x（数秒）
    end
...
这是 **4G 模组 USB 烧录**，不是 T3x GPIO28 烧录。
...
| 下行 | 上行 | 后缀 | 是否秒回 | 是否需 T3x |
|------|------|------|----------|------------|
| 2001 | 1001 | `wakeup` | 是 | 否 |
| 2003 | 1003 | `status` | 是 | 否 |
| 2005 | 1005 | `sim` | 是 | 否 |
| 2008 | 1008 | `version` | 是 | 否 |
| 2006 | 1006 | `identity` | 否（T3x 未就绪则入队） | 是 |
| 2007 | 1007 | `tfcard` | 否 | 是 |
| 2020–2031 | 1020–1031 | `encode` / `record` / `framerate` / `personDetect` / `mic` / `softPhoto` | 否 | 是 |

Lua 入队集合：`user/net_mqtt.lua` `HOST_DL_NEEDS_T3X`（2006/2007/2009/2020–2031）。T3x 没上电时这些命令 **超时是预期**，不能判 Broker 失败。
...
4. 看结果表：通过 / 超时 / 耗时。T3x 未上电时 2006 及之后带 T3x 的条目标「超时」可接受。
...
`--run-safe` 只跑安全集，与 GUI 自动测试同源。`--run-all` 含 extra / danger（格式化、重启等），流程与实机见 [MQTT_ALL_CMD_FLOW_TEST.md](MQTT_ALL_CMD_FLOW_TEST.md)。
...
| 6 | T3x 已上电时再看 2006/2007/2020… | 对应 1006/1007/1020… 在 20s 内到达 |
...
| 超时（需 T3x） | T3x 休眠/未就绪，命令入队 | **4G 单模组验收时不算失败** |
...
| 2006/2020 超时、2008 成功 | T3x 未上电，属实现行为 |
