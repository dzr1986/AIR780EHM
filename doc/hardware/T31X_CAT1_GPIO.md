# T31x + CAT1（Air780EHM）GPIO 功能一览

> **原理图**：`ps01masch260318.pdf`（LGS `T31xX` Rev V0，2024-04-12）  
> **主控**：君正 **T31xZX**（QFN88，`U8`）  
> **4G 模组**：合宙 **Air780EHM**（`M1`，CAT1）  
> **说明**：T31x 引脚多为 **复用**；下表「原理图网络名」为板上实际使用的功能名，「T31x 球名」为 PDF 上标注的默认复用名。
...
| `boot_key` | 28 | 78 | GPIO28 | BOOT 键 | 长按进烧录，见 [T31X_BURN_MODE.md](T31X_BURN_MODE.md) |
| `coproc_ready` | 29 | 30 | GPIO29 | 协处理器就绪 | 上升沿；与 `t31x_mcu_int` 同脚时需按板级设计取舍 |
...
| `t31x_pwr_wake` | 22 | 19 | GPIO22 | CPU_PWR_EN | T31x 供电使能 + 唤醒同线 |
| `t31x_boot` | **26** | **25** | **CAN_TXD** | **T31x_BOOT** | 进/退烧录 BOOT 脚 |
| `t31x_ota` | 32 | 33 | GPIO32 | USB_DEBUG_EN | USB 切换；**正常运行低、烧录高**（见下 §1.2） |
| `t31x_mcu_int` | 29 | 30 | GPIO29 | MCU_INT_CPU | 低脉冲唤醒 T31x `PB27` |

#### 1.2 `USB_DEBUG_EN`（GPIO32）电平

由 **4G 模组 Luat GPIO32** 驱动原理图网络 **`USB_DEBUG_EN`**，控制 USB 走 T31x Host 还是烧录/调试路径。配置真源：`GPIO_OUT.t31x_ota`（`init_level=0`、`on_level=1`）。

| 场景 | `USB_DEBUG_EN`（GPIO32） | `T31x_BOOT`（GPIO26） | 代码 / 说明 |
|------|--------------------------|----------------------|-------------|
| **4G 上电 / 正常运行** | **低（0）** | **低（0）** | `gpio_util.setupOutput` 用 `init_level`；`t31x_ctrl.start()` 后不改 OTA 脚 |
| **进入 T31x 烧录**（GPIO28 长按 → `enterBootMode`） | **高（1）** | **高（1）** | 断电 500ms 后 BOOT+OTA 置 `on_level`，再 `powerOn`（GPIO22） |
| **退出烧录**（`coproc_ready` → `exitBootMode`） | **低（0）** | **低（0）** | 拉回 `init_level` |
| **USB 恢复**（`AT+USBRESET`） | **高约 300ms → 低** | 不变 | `pulseUsbDebugEn()`，**不是**烧录模式 |

原理图（Sheet7）：进烧录需 **`T31x_BOOT` + `USB_DEBUG_EN` + `CPU_PWR_EN`** 同时为高；正常运行三者均为低（`CPU_PWR_EN` 在 T31x 上电时为高，与 USB 切换脚无关）。

板端读电平（4G 侧，Luat GPIO 号）：

```text
gpio.get(32)   # 正常运行期望 0；烧录中期望 1
gpio.get(26)   # T31x_BOOT，烧录时同为 1
```

详细时序：[T31X_BURN_MODE.md](T31X_BURN_MODE.md) · USB 拔出与休眠：[T31X_USB_HOSTIDLE.md](../power/T31X_USB_HOSTIDLE.md) · 低功耗决策：[POWER_USB_BATTERY_T31X_LOGIC.md](../power/POWER_USB_BATTERY_T31X_LOGIC.md) · T31x 启停振荡分析：[T31X_BATTERY_USB_T31X_OSCILLATION.md](../power/T31X_BATTERY_USB_T31X_OSCILLATION.md)（T31x 侧 PA9/VBUS 硬件见 IPC 仓 `docs/gpio_led_config.md`）

#### 烧录 / 协处理器（常用子集）

| 功能 | Luat GPIO | 模组 Pin | 丝印 | `config` 键 |
|------|-----------|----------|------|-------------|
| T31x_BOOT | **26** | **25** | CAN_TXD | `GPIO_OUT.t31x_boot` |
| USB_DEBUG_EN | 32 | 33 | GPIO32 | `GPIO_OUT.t31x_ota` |
| CPU_PWR_EN | 22 | 19 | GPIO22 | `GPIO_OUT.t31x_pwr_wake` |
| 烧录按键 | 28 | 78 | GPIO28 | `GPIO_IN.boot_key` |

详细时序：[T31X_BURN_MODE.md](T31X_BURN_MODE.md) · 配置字段：[CONFIG.md](../overview/CONFIG.md)
...
**烧录说明（原理图 Sheet7 注释）**：长按 K4 约 5s → `T31x_BOOT`、`USB_DEBUG_EN`、`CPU_PWR_EN` 置高 → 进入烧录；完成后双击 K4 或断电重启。

### 2.6 UART — 与 CAT1 / 调试


| 原理图网络名        | T31x 球名           | 引脚# | 连接对象                     | 功能说明         |
| ------------- | ---------------- | --- | ------------------------ | ------------ |
| UART1_TXD     | UART1_TXD `PA06` | 23  | Air780EHM **RXD**（Pin17） | T31x 发 → 4G 收 |
| UART1_RXD     | UART1_RXD `PA07` | 22  | Air780EHM **TXD**（Pin18） | 4G 发 → T31x 收 |
...
| NET_STAT_LED | T31x **VDDIO2** GPIO（Sheet3） | 网络状态灯；**不接 Air780**，见 Sheet6 跨页至 T31x |
...
| MCU_INT_CPU    | **`PB27`**（VDDIO3，`t3x_linux` `wake_gpio=59`） | Cat.1 **GPIO29** 低电平脉冲输入（T31x 上拉 3.3V + 下降沿）；780 `t31x_mcu_int`；供电仍用 **GPIO22** `t31x_pwr_wake` |
| T31x_BOOT       | `BOOT_SEL0` `PC00` 等 | 启动/烧录模式控制          |
...
### 2.11 未在原理图明确连线（T31x 可用复用）

以下为 PDF 引出但未在本板标具体网络名的典型 GPIO（开发时查君正 T31x SDK `gpio` 表）：
...
> 模组 `**M1`**，与 T31x 及外围连接见原理图 **Sheet 7**。
...
| 17      | UART1_RXD      | ← T31x **UART1_TXD**（`PA06`） |
| 18      | UART1_TXD      | → T31x **UART1_RXD**（`PA07`） |
| 19      | GPIO22         | **`CPU_PWR_EN`**（`t31x_pwr_wake`） |
| 20      | PWM1           | PWM 输出                      |
| 22      | PWM0           | PWM 输出                      |
| 23      | ONEWIRE        | 单总线（若用）                     |
| 24      | VDD_EXT        | 模组外部供电（由 T31x 侧电源网络供电）       |
| **25**  | **CAN_TXD**    | 复用 **Luat GPIO26** → 原理图 **`T31x_BOOT`**（`t31x_boot`） |
| 26      | PWM4           | 与 Luat GPIO26 **不同**（勿与 Pin25 混淆） |
| 28–29   | UART2_RXD/TXD  | 第二路串口（本图未接 T31x）             |
| **30**  | **GPIO29**     | **`MCU_INT_CPU`**（Cat.1 唤醒脉冲 → T31x `PB27`） |
| **31**  | **GPIO30**     | **PIR_MCU_DET**，PIR 人体检测输入（固件上升沿） |
| **32**  | **GPIO31**     | **BAT_ALRT_N** → CW2015 Alert（跨页 Sheet6） |
| **33**  | **GPIO32**     | **`USB_DEBUG_EN`**（`t31x_ota`，USB 切换） |
...
| 61      | VBUS           | USB VBUS 检测                 |
| 66–67   | I2C1_SDA/SCL   | 模组 I2C1（本板未接 T31x I2C）       |
| 78      | GPIO28         | 通用 GPIO                     |
| 79      | USIM_DET       | SIM 卡检测                     |
...
| 电池状态灯（4G 侧） | 子板 FPC **BATSTAT_LED**（Sheet6→M1） | **GPIO21**（Pin107，`LED_BLUE`），见 [`CHARGE_BATTERY.md`](../power/CHARGE_BATTERY.md) |
...
> 若需与 LuatOS / T31x SDK 引脚号一一对应，请以君正 **T31x GPIO 编号表** + 本表「T31x 球名」联合查 `hal`/`gpio` 配置。
