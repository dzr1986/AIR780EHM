...
本文说明 **Air780EHM** 侧 PIR 硬件引脚、中断检测、业务联动与 **MQTT 1010** 上报流程。云端策略与 JSON 字段见 [`PIR_PROTOCOL.md`](../pir/PIR_PROTOCOL.md)。

> 一次 PIR → **`APP_PIR_WAKE_T31X`** 一次唤醒；`both` 由 T31x 同周期先拍后录。见 [T31X_RECORD_MQTT_FLOW.md](../pir/T31X_RECORD_MQTT_FLOW.md)。
...
PIR 传感器输出经子板 **FPC J1 Pin3（PIRMCU_DET）** 接到模组 **GPIO30**；检测到人体时输出 **高电平脉冲**，固件配置为 **上升沿** 触发。触发间隔由 **`config.lua` → `cooldown_ms`** 软件节流（子板无 BISS0001 类延时芯片），见 [`PIR_TRIGGER_INTERVAL.md`](../pir/PIR_TRIGGER_INTERVAL.md)。
...
flowchart TB
    ...
    APP[user/app.lua 唤醒 T31x / MQTT]
    NET[user/net_mqtt.lua 1010 / 1011]
    HW --> PIR
    PIR -->|PIR_HW_TRIGGERED| CTRL
    CTRL -->|PIR_WAKE_T31X / STOP / T31X_RECORD_*| APP
    APP --> NET
...
| 编排 | `app.lua` | 订阅事件 → `pubWakeup`、T31x 唤醒、`pubPirDetect` |
...
| `cooldown_ms` | `PIR_COOLDOWN_MS.frequent`（3s） | 冷却期内忽略重复触发；详见 [`PIR_TRIGGER_INTERVAL.md`](../pir/PIR_TRIGGER_INTERVAL.md) |
...
| 完整分析 | **[`PIR_TRIGGER_INTERVAL.md`](../pir/PIR_TRIGGER_INTERVAL.md)**（行业参考、现场日志、选型建议） |
...
| `PIR_WAKE_T31X` | `uploadMode=auto` 时 **1001** + `requestT31xWake()`（photo/video/both 各仅一次） |
| `T31X_RECORD_ACTIVE` | **1010** `t31x_active`（T31x `AT+RECORD=1`） |
| `T31X_RECORD_STOP` | **1011** `source=t31x` |
| `PIR_STOP_RECORDING` | **1011** `source=4g`；可选 `requestT31xWake(pir_stop)` |
...
| `PIR_WAKE_T31X` | `pir_ctrl` | 一次 PIR 一次唤醒 T31x（含 photo/video/both） |
| `T31X_RECORD_ACTIVE` / `T31X_RECORD_STOP` | `host_uart` | T31x `AT+RECORD=` → MQTT 1010/1011 |
...
- [`PIR_PROTOCOL.md`](../pir/PIR_PROTOCOL.md) — 2010 / 2011 / 1010 / 1011  
- [`MQTT_DOWNLINK.md`](../mqtt/MQTT_DOWNLINK.md) §8 — 下行 PIR 配置示例  
- [`CONFIG.md`](../overview/CONFIG.md) — 配置分层  
- [`config.lua`](../../user/config.lua) — `PIR_CFG`  
- [`pir_ctrl.lua`](../../user/pir_ctrl.lua) — 媒体/录像默认策略  
- [`PIR_TRIGGER_INTERVAL.md`](../pir/PIR_TRIGGER_INTERVAL.md) — **触发间隔分析（可视门铃参考）**  
