...
> **双端对齐**：T31x 用 `build/config.mk` 宏管**编译能力**，4G 用 `FEATURE_CFG.low_power` 管**运行策略**。两处开关注释同名，改产品能力时**两边一起改**。  
> **MQTT / TCP / rest 分工**：[CAT1_LOWPWR_MQTT_TCP_STRATEGY.md](CAT1_LOWPWR_MQTT_TCP_STRATEGY.md)  
> **2002 先停 IPC 再断电（时序）**：[MQTT_2002_IPCPOWEROFF_T31_FLOW.md](../mqtt/MQTT_2002_IPCPOWEROFF_T31_FLOW.md)

---

## 1. 开关对照表

| 能力 | T31x（编译） | 4G（Luat 运行） | 说明 |
|------|------------------|-----------------|------|
...
### T31x `build/config.mk`

```makefile
# 与 4G config.lua LOW_POWER_ENABLE 保持一致
WITH_T31X_LOW_POWER ?= yes   # yes | no
```

编译注入：`-DWITH_T31X_LOW_POWER=1/0`（见 `Makefile`）。
...
```lua
-- 与 ipc_device_gb28181 build/config.mk WITH_T31X_LOW_POWER 保持一致
...
_G.LOW_POWER_CFG = {
    ...
    graceful_ipc = true,           -- 需 IPC WITH_T31X_LOW_POWER=yes
...
### 2.1 USB 插入与 T31x 休眠互斥（780EHM_PJ）
...
| 拒绝 T31x **T31x 断电** | `AT+HOSTIDLE=1` → `+HOSTIDLE:USB`（非 BUSY） |
| **主动通知 T31x** | 串口下发 `+CAT1:USB,1`；T31x 停止 HOSTIDLE 轮询 |
| USB 拔出 | `+CAT1:USB,0`；T31x 恢复轮询，满足 `has_event=0` 时可 `HOSTIDLE=1` |

配置：`user/config.lua` → `HOST_USB_CFG`（`block_host_idle_when_usb` / `notify_t31x_usb_state`）。

**USB RNDIS 恢复与 rest**：T31x mismatch 时发 `AT+USBRESET` 仅重绑 RNDIS，**不** `requestT31xWake`；T31x 已 `enterSleep` 断电则 IPC 停发 AT。详见 [T31X_USB_HOSTIDLE.md](T31X_USB_HOSTIDLE.md) §7、T31x `docs/usb_4g_recovery.md` §11。
...
|------|------|
| `usb` | 1=座子插入 |
| `host_idle_allow` | 0=T31x 不应发 `HOSTIDLE=1`；1=允许 |
...
sequenceDiagram
    ...
    participant T as T31x host_event
...
**rest** = `APP_RUNTIME.low_power_mode = 1`：4G **仍联网/MQTT**，T31x **GPIO22 断电**。
...
flowchart TD
    ...
    L --> M[requestT31xWake force_wake]
    M --> N[GPIO 唤醒 / ntfHost]
```
...
¹ **`usb_remove`**：仅 **未开 battery_guard** 的旧逻辑；默认 **battery 策略**下拔 USB **>20% 不进 rest**，见 [WORK_MODE_BATTERY_20PCT.md §9](../_audit/WORK_MODE_BATTERY_20PCT.md#9-usb-拔插与-rest仅电量-20)。  
...
|----------|----------------|------|------|
| **1001** | `.../wakeup` | 事件 | 设备常电在线，可执行业务 / 可唤醒 T31x |
| **1002** | `.../rest` | 事件 | 进入 rest（T31x 断电，4G 仍联网） |
...
**为何 rest 不发 1001？** 旧逻辑 conack 固定发 1001，rest 重连时后台会误以为「已唤醒」，与 T31x 实际断电矛盾。
...
① app 启动
   ...
   → low_power_mode=1，T31x enterSleep 断电
...
字段类型与 [MQTT_PROTOCOL.md](../mqtt/MQTT_PROTOCOL.md) §5.3 一致：`usbInserted` / `charging` 为 **数字** 0/1；`remainPower` / `batteryMv` 为字符串（或 `"--"`）。周期默认 **30s**（`low_power_interval_sec`，非 60s）。
...
| 层级 | 条件 | 行为 | 计数 |
|------|------|------|------|
| suspend | 电量 ≤15% / 烧录 | 停 PIR 业务，T31x **可能仍上电** | `cnt_biz_ignore_suspend` |
| rest | `low_power_mode=1` | 丢弃 PIR，**不** `requestT31xWake`、**不** MQTT 1001 | `cnt_biz_ignore_rest` |
...
退出时 `requestT31xWake(..., { force_wake = true })` 绕过 rest 门禁但仍受 USB/烧录约束。
...
| `t31x_policy` | `mayPowerT31x` 拒绝非 `force_wake` 唤醒 |
| `t31x_policy` | `shouldWakeOnMqttOffline` 返回 false |
| MQTT | `pubRest` 1002，`low_power_mode=rest` |
| `net_tcp` | 关闭数据通道 |

---

## 5. T31x 侧实现要点（`WITH_T31X_LOW_POWER`）

| 文件 | 行为 |
|------|------|
| `app/cat1/types.h` | 默认 `WITH_T31X_LOW_POWER 1` |
...
效果：不进 rest、USB 拔出不断 T31x、PIR 常响应、`AT+LOWPOWER` 返回 `NOT_SUPPORTED`。

### 仅断 T31x、不要 T31x 优雅关机
...
T31x 可保持 `WITH_T31X_LOW_POWER=yes`（其它场景仍可用 IPCSTATUS）；`enterSleep` 直接 `powerOff()`。
...
- [ ] rest 中 PIR 不唤醒 T31x（`ignore_rest` 日志）；无 1001 / 1010
- [ ] `AT+LOWPOWER=ENTER/EXIT` 在开关关闭时 `NOT_SUPPORTED`
- [ ] IPC `WITH_T31X_LOW_POWER=no`：`AT+IPCPOWEROFF` → `NOT_SUPPORTED`
...
| [POWER_USB_BATTERY_T31X_LOGIC.md](POWER_USB_BATTERY_T31X_LOGIC.md) | 模块职责与决策 |
| [T31X_IPC_4G_INTERACTION.md](../t31x/T31X_IPC_4G_INTERACTION.md) | T31x↔4G 端到端流程 |
| [T31X_HOSTEVT_SLEEP.md](../t31x/T31X_HOSTEVT_SLEEP.md) | HOSTEVT 四条 AT：汇总 / 消费 / 休眠 |
| [T31X_NAMING.md](../overview/T31X_NAMING.md) | `t31x` / `T31x` / `T31X` 命名 |
