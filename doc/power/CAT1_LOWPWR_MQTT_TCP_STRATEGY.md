...
> **关联**：[T31X_LOW_POWER.md](T31X_LOW_POWER.md) · [MQTT_PROTOCOL.md](../mqtt/MQTT_PROTOCOL.md) · [MQTT_HOST_CONFIG_MODES.md](../mqtt/MQTT_HOST_CONFIG_MODES.md)
...
T31x bootstrap 可能发送与 `MQTT_CFG` 相同的 `AT+MQTTCFG`。4G 在 `app.lua` `on_mqtt_cfg` 中调用 `net_mqtt.sameMqttCfg`：**参数不变则跳过重连**。
