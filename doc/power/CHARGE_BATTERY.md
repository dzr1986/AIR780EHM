...
> **指示灯专篇**（两套灯规则、场景对照、调试）：[LED_INDICATORS.md](../hardware/LED_INDICATORS.md)

本文描述 **T31x + Air780EHM** 工程中：USB 插入检测、充电状态、**两套指示灯**、**ADC1/BAT_ADC** 采样电量，以及 **MQTT 1003** 上报的完整流程。硬件引脚详见仓库根目录 [`T31X_CAT1_GPIO.md`](../hardware/T31X_CAT1_GPIO.md) §3.1。
...
| **电池电压** | ADC1 `BAT_ADC` | 固件周期采样 | `user/vbat.lua`（自包含 ADC 采样与电量映射） |
...
> 完整规则见 **[LED_INDICATORS.md](../hardware/LED_INDICATORS.md)**（本板 **单蓝灯 GPIO21**）。
...
见 [LED_INDICATORS.md](../hardware/LED_INDICATORS.md)：**闪2下 / 常亮 / 慢闪 / 快闪**。  
...
完整表见 [CONFIG.md §BATTERY_CFG](../overview/CONFIG.md#batterycfg-字段一览)。摘要：
...
| `filter.*` | 见 [CONFIG.md](../overview/CONFIG.md) | **ADC 滤波**：trimmed mean + EMA + 满电滞回，抑制 mV/% 跳变 |
...
现场标定（本板，真源 `config.lua`）：

| 参数 | 值 | 说明 |
|------|-----|------|
| `adc.mv_scale` | `3326/1131` | `mv_scale = 万用表电芯mV / 日志 pin_mV` |
| `cell.v_max_mv` | `4200` | 满充 4.2V |
| `cell.v_min_mv` | `3000` | 截止约 3.0V |

复标：插 USB 充电时建议在稳定后读表；改 `config.lua` 后重启生效。
...
- [`T31X_CAT1_GPIO.md`](../hardware/T31X_CAT1_GPIO.md) — 引脚与 LP4030 硬件说明  
- [`MQTT_PROTOCOL.md`](../mqtt/MQTT_PROTOCOL.md) — 1003 等数据类型  
- [`PIR_HARDWARE.md`](../hardware/PIR_HARDWARE.md) — GPIO30 / PIR_MCU_DET 人体检测  
- [`KEY_GPIO.md`](../hardware/KEY_GPIO.md) — GPIO 按键（`peripheral` + `gpio_cfg` `KEY_CONFIG`）  
- [`CONFIG.md`](../overview/CONFIG.md) — 配置分层与引脚总表  
