...
| T3x | **PB27** | 3.3V 输入 | 内部/外部**上拉至 3.3V**，`gpio.c` **下降沿**触发 |

**为何不用高电平脉冲**：1.8V 输出高通常达不到 T3x 3.3V CMOS 的 VIH（约 2.0V+），高脉冲不可靠；**拉低**时两端都能识别为有效低。

**硬件**：GPIO29 与 PB27 之间建议有电平转换或至少保证 PB27 有 3.3V 上拉；仅直连时依赖 T3x 上拉判空闲高、4G 拉低判脉冲。
...
2. T3xx：PB27 下降沿 → `AT+HOSTEVT?`  
3. 780：`\r\n+HOSTEVT:sid,evt\r\nOK\r\n`（**仅查询，不清除 pending**）  
4. T3xx：执行业务后 → `AT+HOSTEVTCLR` → `+HOSTEVTCLR:OK`  
5. T3xx：`evt=0` 业务回调；`evt=1/2/3` 重建 `SERVCREATE` 通道  
