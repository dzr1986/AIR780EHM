# T3x Linux ↔ Air780 4G：AT 交互与 PIR 状态查询
...
> 唤醒 GPIO：见 [T3X_HOSTEVT_PROTOCOL.md](T3X_HOSTEVT_PROTOCOL.md)
...
| 侧 | 职责 |
|----|------|
| **T3x** | TCP 业务通道参数、MQTT Broker、查询 4G 运行态/PIR 统计、收到 GPIO 脉冲后读 `HOSTEVT` |
| **4G** | 蜂窝/MQTT、PIR 硬件与策略、录像会话、冷却节流、**保存 PIR 判断计数**、主动 GPIO 唤醒 T3x |
...
> `SERVCREATE` 与 MQTT 独立：前者为 T3x 侧 TCP 长连接模板（当前 4G 脚本主要存配置）；MQTT 由 `net_mqtt.lua` 连接 Broker。
...
T3x API：`client_get_runtime_config`、`client_query_host_evt`、`client_get_pir_stat`、`client_sync_time_from_cat1`。
...
## 3. 4G → T3xx：GPIO 唤醒 + HOSTEVT

除 GPIO 脉冲外，4G 还可 **主动下发** AT（T3x `uart_host_cmd.c` 解析）：
...
T3x 流程：`gpio` 中断 → `AT+HOSTEVT?` → `client_handle_event` → 业务回调或重建 `SERVCREATE` + `MQTTCFG`。
...
PIR 判断**可以且应当保存在 4G**（传感器接在 Cat.1 GPIO30）。T3x 通过 `AT+PIRSTAT?` 读取，无需在 T3x 再实现一套冷却/录像状态机。

### 4.1 各分支判定与 4G 侧持久化

**结论：PIR 各分支的「是否触发 / 为何忽略」应保存在 4G**（RAM 计数 + 当前会话快照），T3x 用 `AT+PIRSTAT?` 读取，不必在 T3x 复刻冷却与录像状态机。

| 阶段 | 条件 | 4G 动作 | `last=` | 累加计数 |
|------|------|---------|---------|----------|
| 硬件 | T3x 烧录模式 | 忽略中断 | `ignore_burn` | `cnt_hw_ignore_burn` |
...
| 业务 | 正常检测 | 拍照/录像 + 唤醒 T3x | `detected` | `cnt_biz_detected`, `cnt_biz_photo`/`cnt_biz_video` |
...
pir_ctrl.onPirTriggered
  ├─ suspended (T3x 烧录等)     → cnt_biz_ignore_suspend, last=ignore_suspend
  ├─ 录像中且 stopOnSecondPir   → cnt_biz_retrigger, 停录, last=retrigger
  └─ 正常检测                   → cnt_biz_detected, last=detected
        ├─ action=photo/both    → cnt_biz_photo, 唤醒 T3x(evt=0), MQTT 1010
        └─ action=video/both    → cnt_biz_video, 录像会话, 唤醒 T3x
录像停止
...
| 通道 | 用途 |
|------|------|
| **UART `AT+PIRSTAT?`** | T3x 本地策略、UI、日志、与 evt 无关的**累积统计** |
...
| `burn_mode` | 1=`T3X_BURN_MODE_ACTIVE` |
...
| [T3X_HOSTEVT_PROTOCOL.md](T3X_HOSTEVT_PROTOCOL.md) | GPIO 唤醒 |
