...
> **已取代（2026-08-17）**：低电自动进 PIR 已取消。现行规范见 **[WORK_MODE_PERSON_DETECT_PIR.md](../power/WORK_MODE_PERSON_DETECT_PIR.md)**（开机人形常电；仅 MQTT 2002 才断 T31 用 PIR）。下文仅作历史对照。

> **2026-06-30 更新**：默认 `battery` 策略为三档（>20% 常电 · 5~20% HOSTIDLE · ≤5% 关机），5~20% **不进 4G rest**。见 [LUA_MODULES.md](../overview/LUA_MODULES.md) §5.1。下文 ≤10% rest 描述适用于 `hybrid` 或历史对照。

> **产品规则**  
> - **① 一直录像模式**：全程录像至 SD 卡 + 动态侦测弹窗报警  
> - **② 动态侦测模式**：PIR 触发录像至 SD 卡 + 动态侦测弹窗报警  
> - **一直录像模式下**：电量 **≤20%** → 低功耗（= 动态侦测）；电量 **>20%** → 恢复一直录像  
> **代码**：`user/battery_guard.lua` · `user/config.lua` · `user/pir_ctrl.lua` · `lib/t31x_policy.lua` · `user/host_uart.lua`  
> **关联**：[LOW_BATTERY_AND_LOW_POWER.md](../power/LOW_BATTERY_AND_LOW_POWER.md) · [BATTERY_REST_SWITCH_CONDITIONS.md](../power/BATTERY_REST_SWITCH_CONDITIONS.md) · [PIR_PROTOCOL.md](../pir/PIR_PROTOCOL.md) · [T31X_IPC_4G_INTERACTION.md](../t31x/T31X_IPC_4G_INTERACTION.md)
...
T31x **每 30 秒**空闲轮询（`HOSTEVTPOLL=30000`）触发的 `HOSTIDLE` 断电，与 **电量 20% rest** 是 **两条通路**，默认 **不矛盾**：

- **默认策略 `battery`**：
  - **IPC**：电量 **>20%** 时 **不发** `HOSTIDLE=1`（先 `GETCFG` 读 `battery=`）
  - **4G 兜底**：若仍收到 `HOSTIDLE=1` → `+HOSTIDLE:BUSY`
  - T31x **保持常电** 全天录；仅 **≤20%** 走电量 rest / HOSTIDLE 断电
- 需切换策略时改 `config.lua` 顶部 `LOW_POWER_ENTER_STRATEGY`（`battery` / `hybrid` / `idle_poll`）。

详见 **[LOW_POWER_ENTER_STRATEGY.md](../power/LOW_POWER_ENTER_STRATEGY.md)**。
...
|------|----------------------|----------|------|
| **① 一直录像** | `1` | T31x **常电**，TF 卡 **全天连续 MP4** | PIR / IVS → **GB28181 弹窗报警** |
| **② 动态侦测** | `3` | 仅 **PIR 触发** 限时录像至 SD | 同上 |

平台可通过 MQTT **2010** 配置 PIR 策略（`action=video/photo` 等）；`record_mode` 由 T31x 本地配置或远程下发决定。

**电量不改变 `record_mode`**；电量只决定 T31x **常电还是断电 rest**，以及 rest 下是否允许 PIR 唤醒。
...
| 电量 | 产品表现 | 技术映射 |
|------|----------|----------|
| **>20%** | **一直录像模式** | T31x **常电**；全天 SD 录；`HOSTIDLE=1` → **BUSY**（拒绝休眠） |
| **≤20%** | **低功耗 = 动态侦测模式** | `enterBatteryRest` → `IPCPOWEROFF` → **断 T31x 电**；PIR **仍可唤醒** 限时录 + 报警 |
| **>20%（恢复）** | 恢复 **一直录像** | `exitBatteryRest` → `requestT31xWake(force)` → 全天录恢复 |
| **USB 插入** | 充电优先 | 忽略电量阈值，退出 rest，T31x 上电 |
...
| 配置键 | 默认 | 含义 |
|--------|------|------|
| `t31x_rest_percent` | **20** | ≤20% 进入 rest（低功耗=动态侦测） |
| `recover_rest_percent` | **20** | >20%（≥21%）退出 rest，恢复一直录像 |
| `pir_suspend_percent` | **10** | ≤10% 挂起 PIR |
| `block_wake_below_percent` | **20** | ≤20% 非 PIR 原因不唤醒 T31x |
...
详解：[BATTERY_REST_SWITCH_CONDITIONS.md](../power/BATTERY_REST_SWITCH_CONDITIONS.md)
...
4G（本仓库）
  ...
  ≤20% + 条件满足 → onEnterLowPower → t31x_ctrl.enterSleep()
  >20% + 条件满足 → onExitLowPower → requestT31xWake(force)
  rest 下 PIR → pir_ctrl 不 ignore → t31x_policy 允许唤醒

T31x IPC（ipc_device_gb28181）
...
IPC 通过 **`AT+GETCFG` 的 `battery=`** 判断是否 >20%（`app/cat1/host_event.c`），无需本地 ADC。详见 [LOW_POWER_ENTER_STRATEGY.md §3](../power/LOW_POWER_ENTER_STRATEGY.md#3-ipc-侧电量-20-禁止-hostidle双端防护)。

需开启 `WITH_T31X_LOW_POWER=1`、`WITH_T31X_HOSTEVT_SLEEP=1`。
...
| # | 条件 | 预期 |
|---|------|------|
| 1 | `record_mode=1`，电量 30% | T31x 常电，全天录，`lowpower=0` |
| 2 | 降至 18%（连续 2 次） | `lp+ battery`；T31x 断电；MQTT `rest` |
| 3 | 18% 触发 PIR | T31x 唤醒、限时录、报警；结束后再断电 |
| 4 | 回升至 21%（连续 3 次，且 rest≥10min） | `lp- battery_recover`；恢复全天录 |
| 5 | 21% 空闲 30s 轮询无事件 | IPC **不发** HOSTIDLE；日志 `skip HOSTIDLE (battery ... > 20%)` |
| 5b | 21% 若误发 HOSTIDLE | 4G 回 **BUSY**，T31x 保持上电 |
...
> **关联**：[LOW_BATTERY_AND_LOW_POWER.md §场景 B](../power/LOW_BATTERY_AND_LOW_POWER.md#场景-b用户从座上拿走设备拔-usb)
...
| `lowPowerMode` | 4G **业务 rest** | `rest` 时 T31x 通常断电 |
...
flowchart TD
    ...
    H --> J[T31x 断电 + MQTT 1002 reason=battery]
```
...
GPIO27 插入 → exitRestIfNeededAfterUsbInsert → battery_guard.onUsbInserted()
  → 退出 rest（reason=usb_insert）→ requestT31xWake(force)
...
evaluate() → tryExitMismatchedRest()
  ...
  → onExitLowPower("battery_recover") → 拉起 T31x
```
...
| 2026-06-26 | v1.2：与 LOW_BATTERY / T31X_USB_HOSTIDLE 交叉引用对齐 |
