...
| **电池电压** | ADC1 `BAT_ADC` | 固件周期采样 | `user/vbat.lua`（编排 `adc_lib` + `bat_core`；`bat_adc.lua` 仅为兼容桩） |
...
`app.lua` 经 `optMod("battery", "vbat")` 加载；`vbat` 内部 `require "adc_lib"`、`require "bat_core"`。`bat_adc.lua` 仅为 `return require "vbat"` 兼容桩。
...
- [`KEY_GPIO.md`](KEY_GPIO.md) — GPIO 按键（`lib/key` + `key_config`）  
