...
| **模组电量灯** | GPIO20 红 / **GPIO21 蓝（BAT_STAT_LED）** | 固件 `led_ctrl` | 子板 FPC J1 Pin4 → 主板 → GPIO21；依据 `APP_RUNTIME.battery_percent` |
...
阈值在 `led_ctrl` 的 `LED_CONFIG.battery`：`high_threshold = 70`，`medium_threshold = 20`。
