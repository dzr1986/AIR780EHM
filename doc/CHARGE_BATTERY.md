...
| **模组电量灯** | GPIO20 红 / **GPIO21 蓝（BAT_STAT_LED）** | 固件 `ledCtrl` | 子板 FPC J1 Pin4 → 主板 → GPIO21；依据 `APP_RUNTIME.battery_percent` |
...
| `GPIO_OUT.bat_stat_led` | **21** | **BAT_STAT_LED** | 输出 | 中高电量蓝灯，`ledCtrl` 写 1 点亮（板级若反相以实测为准） |
...
### 3.2 模组电量灯（`../user/led_ctrl.lua`，读 `APP_RUNTIME.battery_percent`）

| 电量（ADC 换算 `%`） | GPIO20 红 | GPIO21 蓝 |
|----------------------|-----------|-----------|
| **> 70%** | 灭 | **常亮**（约 10s 一轮） |
| **20% ~ 70%** | 灭 | **闪烁**（亮 1s / 灭 1s，5 次后间隔） |
| **≤ 20%（低电量）** | **闪烁**（250ms 节奏） | 灭 |
| 未知 / 非数字 | 按 `unknown_hold` 等待 | |

阈值在 `ledCtrl` 的 `LED_CONFIG.battery`：`high_threshold = 70`，`medium_threshold = 20`。

**说明**：插 USB 充满时，充电板多为 **蓝灯**；若 ADC 仍算到 ≤20%，模组仍会 **红闪**——需用万用表/日志核对 `cell.v_max_mv`/`cell.v_min_mv` 标定。
...
| `gpio = true` | 需要模组 LED/PIR 时 | 内含 `ledCtrl` |
...
2. `setupGpio()` — 启动 `ledCtrl`（读后续更新的 `APP_RUNTIME.battery_percent`）
...
| 字段 | 来源 |
|------|------|
| `powerStatus` | `APP_RUNTIME.power_status`（USB 插入 ≈ 1，见 `applyUsbInsertState` / `initPowerStatus`） |
| `remainPower` | `APP_RUNTIME.battery_percent`（字符串，ADC 百分比） |
...
| `BATTERY_UPDATE` | bat_adc.lua | `percent, mV, rate` | app 日志；ledCtrl 间接读 `APP_RUNTIME.battery_percent` |
...
- [`T31_CAT1_GPIO.md`](../T31_CAT1_GPIO.md) — 引脚与 LP4030 硬件说明  
