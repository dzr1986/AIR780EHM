# 电量 · USB · 低功耗 · T31 启动 — 逻辑关系总览

> 解决「低电量 / 低电量+USB / 上电默认 USB / 何时启动 T31」混在一起不好理解的问题。  
...
| **④ T31 供电** | `t3x_ctrl` 内部 `isPoweredOn` | GPIO22 高/低 | **硬件上** T31 是否有电（与 ③ 可能不一致） |
...
- `lowPowerMode: "rest"`（1003）≠ T31 一定断电（可能 flag=1 但 GPIO22 仍高）。
- 插 USB **不会** 立刻把 `remainPower` 拉高，只 **取消** 低电量断 T31/关机策略。
- 模组 **红灯闪** 只看电量，**不看** USB。
...
| 优先级 | 条件 | 对 T31 / 电量保护 |
|--------|------|-------------------|
| **P0** | T31 烧录模式 `T3X_BURN_MODE_ACTIVE` | 暂停 `battery_guard`；专用心跳/供电时序 |
| **P1** | **USB 插入** `power_status==1` | `battery_guard` **整段跳过**；插入时 **wake T31**、恢复 PIR、取消 ≤5% 关机 |
| **P2** | 未插 USB + **电量** | ≤15% 停 PIR；≤10% 进低功耗+断 T31；≤5% 延时关机 |
| **P3** | **业务低功耗**（USB 拔出 / MQTT 2002 / AT） | `onEnterLowPower` → 断 T31 + 1002（与电量可叠加） |
  ...
  → app.start()
       ...
       → initPowerStatus()        -- 读 GPIO27
            ...
            若 USB=0 且 charge=false 且 mqtt=true：
                 setLowPowerMode(1) + enterSleep()（仅断 T31，未必发 1002）
...
| 步骤 | 行为 |
|------|------|
| `t3x_ctrl.start()` | **T31 上电** |
| `initPowerStatus()` | `power_status=1`，**不**自动 `enterSleep` |
| `battery_guard.evaluate` | 见 USB → **return**，不按电量断 T31 |
| 结果 | **T31 一直运行**（除非后续云端 2002 enter、烧录、手动断电） |

### 3.2 「默认未插 USB」启动（常见出货态）

| 步骤 | 行为 |
|------|------|
| `t3x_ctrl.start()` | **T31 先上电** |
| `initPowerStatus()` | **不**因无 USB 立刻休眠（`charge=true` 时） |
| 首次 `BATTERY_UPDATE` / `battery_guard` | 若电量 ≤10% → `onEnterLowPower` → **断 T31**；≤15% 仅停 PIR |
| 结果 | 上电瞬间 T31 会亮一下，**几百毫秒～数秒内**可能被电量保护关掉 |

### 3.3 与「用户以为的开机默认」的差异

| 用户预期 | 当前实现 |
|----------|----------|
| 无 USB 出厂即 T31 休眠 | 需 `charge=false` 走 `initPowerStatus` 的 `enterSleep`，或等 `battery_guard` ≤10% |
| 有 USB 才允许 T31 跑 | 插 USB 后 **`battery_guard` 不拦**；但 **`t3x_ctrl.start()` 无 USB 也会先上电** |
| 低电量绝不启 T31 | 仅 **未插 USB** 时由 `battery_guard` 保证；**插 USB 低电仍会 wake/保持上电** |
...
| USB | 电量(未插USB) | low_power_mode | T31 典型状态 | 主要来源 |
|-----|---------------|----------------|--------------|----------|
...
| 0 | ≤10% | 0 | **可能上电** | 仅停 PIR，**未必**断 T31（10%<电量≤15%） |
...
| 0 | ≤10% | 暂停 | 常电 + 1002 rest | T31 断，模组不断网 |
...
| 模块 | 只管什么 | 不管什么 |
|------|----------|----------|
| **`vbat`** | 采样 → `battery_percent` | T31、USB、MQTT |
| **`usb_charge`** | GPIO27 → 发布 `GPIO_USB_DET_CHANGED` | 电量、T31 |
...
| **`app.onEnterLowPower`** | `low_power_mode=1`、断 T31、1002、关 TCP | 是否低电（任何来源都可触发） |
| **`app.onExitLowPower`** | `low_power_mode=0`、wake T31、恢复 TCP 等 | USB 必须插入（通常由 USB 路径调用） |
...
| **`net_mqtt` 2002** | 发布 `POWER_ENTER/EXIT_REST` | T31 引脚 |
...
| # | 现象 | 原因 |
|---|------|------|
| 1 | 无 USB 上电 T31 闪一下再灭 | `t3x_ctrl.start()` **总是** `powerOn`，`battery_guard` 晚 500ms 才断 |
| 2 | `rest` 但 T31 仍重启 | `onMqttOffline` / `notify_host` **不检查** `low_power_mode` 与低电 |
| 3 | `low_power_mode` 与 T31 断电不同步 | `initPowerStatus` 在 `charge=true` 时无 USB **不** `enterSleep`；只靠后续事件 |
| 4 | 电量 12% 边界抖 | `recover_rest_percent=12` 与 `t31_rest_percent=10` 迟滞过窄 |
...
| 编号 | 规则建议 | 对应实现要点 |
|------|----------|--------------|
| R1 | **未插 USB + 电量 ≤ X%**：禁止 T31 上电/唤醒 | 统一门禁函数 `mayPowerT31()` |
| R2 | **插 USB**：允许 T31（即使低电），便于充电/调试 | 保持 `battery_guard` USB 优先 |
| R3 | **上电无 USB**：不要先亮 T31 再灭 | `t3x_ctrl.start()` 改为按 R1 决定是否 `powerOn` |
| R4 | **业务 rest**：MQTT 离线/PIR **不得** 拉起 T31 | `sendWakePulse` / `notify_host` 内检查 |
| R5 | **电量恢复**：加大迟滞，如 ≤10% 休眠、≥20% 才恢复 | 改 `BATTERY_CFG.guard` |
| R6 | **出厂默认**：无 USB 即 T31 休眠 | `initPowerStatus` 与 R3 对齐，或 `charge=true` 时也 `enterSleep` |

### 7.2 固件改造落点（若排期开发）

```text
新增 app/t3x_policy.lua（或 lib/t3x_policy.lua）
  mayPowerT31(reason)  → 综合 USB + 电量 + low_power_mode + 烧录
  requestT31Wake(reason)
  requestT31Sleep(reason)

改造调用点：
  t3x_ctrl.start          → mayPowerT31("boot")
  host_uart.notify_host   → requestT31Wake
  onMqttOffline           → rest/低电时 no-op
  battery_guard           → 仅负责阈值，最终调 requestT31Sleep
  onEnterLowPower         → requestT31Sleep
  onExitLowPower          → requestT31Wake
```
...
| 目标 | 配置 |
|------|------|
| 更早断 T31 | `t31_rest_percent = 15`（与停 PIR 对齐） |
...
flowchart TD
    ...
    Bat -->|否| Bat10{pct ≤ 10%?}
    Bat10 -->|是| Rest[PIR停 + onEnterLowPower<br/>断 T31 + 1002]
    Bat10 -->|否| Bat15{pct ≤ 15%?}
    Bat15 -->|是| PirOnly[仅暂停 PIR]
    Bat15 -->|否| LP{low_power_mode==1?}
    LP -->|是| Sleep[T31 应断电<br/>仅脉冲/notify 需门禁]
...
| 主题 | 文档 |
|------|------|
| 低电量 vs 低功耗、§9 T31 重启、§10 低电+USB | [LOW_BATTERY_AND_LOW_POWER.md](LOW_BATTERY_AND_LOW_POWER.md) |
| 充电/ADC/1003 | [CHARGE_BATTERY.md](CHARGE_BATTERY.md) |
| GPIO27 / USB | [T31_CAT1_GPIO.md](T31_CAT1_GPIO.md) |
| 唤醒脉冲 / WAKEVT | [T31_WAKE_PROTOCOL.md](T31_WAKE_PROTOCOL.md) |
...
若按 §7.2 实现统一 `mayPowerT31()`，应在本节更新「已修复」列表并改代码注释指向本文档。
