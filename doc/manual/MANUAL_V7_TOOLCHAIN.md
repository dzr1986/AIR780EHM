...
| `_dep_graph.py` / `_layer_check.py` | 依赖图（五种边形态、硬环/软环/反向边，`--mermaid` 出图）/ 分层护栏（R1 lib↛user 业务、R2 config 域↛utils 系、R3 子模块↛主文件、**R4 AT 层↛业务层**（基线 15）；基线白名单只许收缩，`--save-baseline`） | 改 `require`/`loader.load` 拓扑后（`run_all_checks` 第 11 项） |
