# T31x 烧录模式（GPIO28 BOOT 键长按）

> 硬件（`ps01masch260318.pdf` Sheet7）：  
> - **T31x_BOOT** → Luat **GPIO26** = 模组 **Pin25**（丝印 `CAN_TXD`）  
...
> 全量 GPIO 对照：[CONFIG.md](CONFIG.md#air780-gpio-编号对照configlua) · [T31X_CAT1_GPIO.md §1.1](T31X_CAT1_GPIO.md#11-780ehm_pj-固件-gpio-对照configlua-真源)
...
按 `app.shutdownServicesForT31xBurn()` 顺序（进入时立即置位 `_G.T31X_BURN_MODE_ACTIVE` 与 `heartbeat_paused`）：
...
| 6 | T31x 时序 | `t31x_ctrl` | `enterBootMode()` |

---

## 3. 烧录时序（`t31x_ctrl.enterBootMode`）

```mermaid
sequenceDiagram
    participant A as app
    participant T as t31x_ctrl
    participant M as T31x
...
原理图：进烧录需 **`T31x_BOOT` + `USB_DEBUG_EN` + `CPU_PWR_EN`** 置高；固件用 **GPIO26 + GPIO32 + GPIO22** 等效。
...
|-----------|----------|------|-------------|------------|
| **26** | **25** | **CAN_TXD** | `t31x_boot` | **T31x_BOOT** |
| **32** | 33 | GPIO32 | `t31x_ota` | **USB_DEBUG_EN** |
| **22** | 19 | GPIO22 | `t31x_pwr_wake` | **CPU_PWR_EN** |
| 28 | 78 | GPIO28 | `boot_key`（输入） | 长按触发烧录流程 |

### 3.2 `USB_DEBUG_EN` / `T31x_BOOT` 电平（烧录 vs 正常）

`config.lua`：`t31x_boot`、`t31x_ota` 均为 `init_level=0`（上电低）、`on_level=1`（烧录高）。

| 状态 | USB_DEBUG_EN GPIO32 | T31x_BOOT GPIO26 |
|------|---------------------|-----------------|
| 正常开机 / IPC 运行 | **低** | **低** |
...
| 勿把 Pin25 写成 Pin26 | Pin26 为 PWM4，与 T31x_BOOT 无关 |
