...
| # | 任务 | 产出 |
|---|------|------|
| P0-1 | 重写 `doc/overview/LUA_MODULES.md` §2 为 **模块树**（host_uart / net_mqtt 子树 + 其余 user 一览） | 单页全景 |
| P0-2 | `doc/modules/README.md` 增加 **子模块索引表**（文件名 → 职责 → 专题 doc 链接） | 与 P0-1 互链 |
| P0-3 | `doc/overview/CALL_GRAPH.md` 补充 bind 顺序：`hif_ipc_*`、`net_mqtt_*` 加载链 | 防顺序回归 |
