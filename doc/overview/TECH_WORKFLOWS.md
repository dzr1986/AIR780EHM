...
| 5 | T31x 录像回链 | T31x `AT+RECORD=1` → `hif_cmd_t31x.uartRecord` → **`setRecActive(1)`**（录像态唯一写点，`PIR_CTRL_FLOW §10`）→ `T31X_RECORD_ACTIVE` ⇢ `pubRecActive`(1010 `t31x_active`)；`AT+RECORD=0,reason=…` → `setRecActive(0)` → `T31X_RECORD_STOP` ⇢ `pubT31xStop`(1011 `source=t31x`) + `pir_ctrl.syncStopT31x` | 1010 / 1011 | `STOP_STAT_KEY` 统计（DEVICE/CLOUD 同键有意） |
...
| 5 | 进 rest | `app.onEnterLowPower(reason)` → `runtime_power.canRest` 预判（提示音前）→ `requestRest`（**PSM 唯一写点**：门禁 enabled/usbBlocks、USER_CUT 不拦 USB）→ app 副作用：`t31x_ctrl.enterSleep` → `net_mqtt.pubRest`(1002) → `lp_wakeup.onEnterRest` | 1002；`POWER_ENTERED_REST(reason)` | `LOW_POWER_WAKEUP_CFG.mode=mqtt/tcp` 决定通道；PSM 见 `LOW_POWER_WAKEUP §2b` |
| 6 | 出 rest | `app.onExitLowPower(reason)` → `runtime_power.requestNormal`（无门禁，changed 判重放）→ `pubRest{exit}` → `lp_wakeup.onExitRest` → `t31x_ctrl.ensNormalPwrOn`+`pulseMcuInt`（即使已 Normal 也执行，清 BOOT/OTA） | 1002 exit；`POWER_EXITED_REST(reason)` | 2002 exit / USB 插入 / 电量恢复 |
