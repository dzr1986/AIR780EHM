...
`ota_server` 使用 **HTTP 拉包** + **780EHM_PJ MQTT 2004/1004**。
...
设备**真正下载固件**使用 HTTP GET。

### 2.1 接口

| 路径 | 说明 |
|------|------|
| `GET /api/site/firmware_upgrade` | 设备拉包路径 |
...
- `Content-Length`：文件字节数（**0 字节绝不返回 200**）
- `Accept-Ranges: bytes`；带 `Range` 时返回 **206**
- `X-Ota-Target-Version`：目标版本
- `X-Ota-Release-Id`：固件条目 ID

版本匹配规则与循环保护见 [OTA_FOTA.md](OTA_FOTA.md)。
...
| **固件库** | `FirmwareRegistryService` | MySQL 固件包 |
...
|------|----------|-----------------|
| HTTP OTA | 模组 HTTP GET | 托管 dfota、按版本匹配 |
